...
- `passive: false` hides every hover tooltip. That now includes the heatmap, treemap, tree, network, sankey, choropleth, unit, bump, dumbbell, parallel-coordinates, distribution, density-scatter and Gantt tooltips, the canvas layer's hover, and any `ChartTooltipBox` you mount yourself.
- `active: false` turns off direct manipulation. The navigator strip stays as a read-only overview with no handles, drag or wheel pan. Pinch, Ctrl/⌘-wheel and `+` / `−` / `0` zoom stop, and the zoom buttons are not shown. Pan and zoom stop on the density scatter, the choropleth and the tree, and so do the tree's minimap clicks, network node drag, Gantt bar drag, Gantt zoom and the selection gestures (range, rectangle, lasso, radial).
