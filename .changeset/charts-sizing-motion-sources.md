...
Charts on the shared measurement path measure their size, pace their redraws during a resize and decide on reduced motion the same way. `TreeChart`, `Sparkline` and `Gantt` still measure themselves and keep their old timing. With no new props set, every chart draws at the same size as before.
