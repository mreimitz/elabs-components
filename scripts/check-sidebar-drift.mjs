 ...
 * (`TeamSwitcher`, `NavMain`, `NavUser`, `NavNotifications`) into `@qlik-coe-emea/qlabs-components-ui`
 ...
 * of importing / re-exporting it from `@qlik-coe-emea/qlabs-components-ui`.
 *
 ...
 *     import { TeamSwitcher } from "@qlik-coe-emea/qlabs-components-ui"                 — usage
 *     export { TeamSwitcher } from "@qlik-coe-emea/qlabs-components-ui"                 — pass-through re-export
 *     export { NavMain as DashboardNavigation } from "@qlik-coe-emea/qlabs-components-ui" — aliased re-export
 *     export type { TeamSwitcherProps } from "@qlik-coe-emea/qlabs-components-ui"       — types only
 *
...
/**
 * Shared nav primitives that now live in `@qlik-coe-emea/qlabs-components-ui` (issue #99). A registry
    ...
    for (const v of findings) {
      const rel =
        v.file && v.file.startsWith(REPO_ROOT)
          ...
          : (v.file ?? "<inline>");
      console.error(
        `  ${rel}:${v.line}  ${v.name} is re-declared locally — import it from @qlik-coe-emea/qlabs-components-ui instead`,
      );
    }
    console.error(
      `\nThe shared nav primitives (${[...GUARDED].join(", ")}) live in @qlik-coe-emea/qlabs-components-ui (issue #99).\n` +
        ...
        `\`import { ${[...GUARDED][0]} } from "@qlik-coe-emea/qlabs-components-ui"\` (or \`export { ... } from "@qlik-coe-emea/qlabs-components-ui"\`).`,
    );
