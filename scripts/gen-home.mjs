...
/**
 * README.md documents which `.json` files live in this directory and what each one contains.
 * Every generated file in buildAll() must be listed here so a future added file can't be
 * silently left off the index (#571).
 */
export function buildReadme() {
  // Each entry: [filename, description]. Must be kept in sync with the files in buildAll().
  const fileDescriptions = [
    [
      "packages.json",
      "Each `@elabs-ai/components-*` package: short name, one-line description, layer, export count, heavy peer engines.",
    ],
    [
      "counts.json",
      "Every number the site shows (packages, component exports, registry blocks, templates, playbooks, theme families, tokens, skills, hosted MCP tools, gates), each with the repo path it was derived from.",
    ],
    [
      "themes.json",
      "The default family plus every downloadable theme family (`themes/*`): display name, whether it ships a typeface, and each mode's resolved `--primary`/`--background`.",
    ],
    [
      "gates.json",
      "Every check rule and external check command: id, one-line rationale, category, source file.",
    ],
    [
      "cli.json",
      "The CLI's standalone verbs (grouped), the hosted vs. local-only MCP tool split, the `info → search → docs → build → audit` routine, and the hosted MCP url/command.",
    ],
    [
      "blocks.json",
      "The registry's blocks: name, title, categories, dependencies, and the best-matching `docs/playbooks` archetype (see the note in `scripts/gen-home.mjs`).",
    ],
    [
      "playbooks.json",
      "The archetype playbooks: intent, keywords, packages, doc file, template file.",
    ],
    [
      "story-ids.json",
      "Component name → Storybook docs page id, for every component with an autodocs page.",
    ],
    [
      "install.json",
      "The install snippets: CLI, hosted MCP, local MCP, plugin marketplace commands, registry homepage, one `pnpm add` per archetype.",
    ],
    [
      "agent-loop-recorded.json",
      "The hosted MCP's answers to every call in the hand-authored `apps/home/content/agent-loop.json`, the agent loop's offline fallback (RM-099).",
    ],
    [
      "emit-ui-examples.json",
      "The A2UI editor's example menu, from the CLI's `a2ui example` and `content/examples/` (RM-101).",
    ],
    [
      "create-theme.json",
      "The `/create-theme` plugin skill's schema: name, slash command, argument hint, and full invocation template (RM-103).",
    ],
    [
      "catalog-index.json",
      "The Storybook catalogue's component/visualization/block index: every item's section, package, family, group, slug, featured rank, and display metadata.",
    ],
    [
      "catalog-nav.json",
      "The catalogue's navigation tree: sections (components, explore), packages, families, and group counts for sidebar rendering.",
    ],
    [
      "catalog-pages.json",
      "Catalogue metadata for every Storybook docs page: title, description, section, family, Storybook story id, and frontmatter field order.",
    ],
    [
      "story-aliases.json",
      "Maps of Storybook story ids for redirection and alias purposes across catalogue sections.",
    ],
    [
      "catalog-redirects.json",
      "Permanent 308 redirects (source → destination) for pages moved within the catalogue; generated from the layout.",
    ],
    [
      "catalog-family-order.json",
      "The display order of component families in the Catalogue's Components section sidebar.",
    ],
  ];

  const header = `# Generated content — do not edit
...
`;
  ...
  const tableHeader = `| File             | What it holds                                                                                                                                                                                           |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
`;

  const tableRows = fileDescriptions
    .map(([filename, desc]) => `| \`${filename}\` | ${desc} |`)
    .join("\n");
...
`;
  ...
  return header + tableHeader + tableRows + footer;
}
...
async function main() {
  ...
  // Write README.md (RM-090: generated index of what files exist in this directory)
  ...
  const formattedReadme = await prettier.format(readmeContent, {
    ...options,
    filepath: readmePath,
  });
  const readmeStale = formattedReadme !== existingReadme;
  if (readmeStale) {
    if (!check) {
      mkdirSync(dirname(readmePath), { recursive: true });
      writeFileSync(readmePath, formattedReadme, "utf8");
    }
  ...
  if (check) {
    if (stale.length > 0) {
      console.error(
        `✖ apps/home/content/generated/*.json and README.md is STALE: ${stale.join(", ")}\n` +
          "  Run `pnpm gen --only home` and commit the result.",
      );
      process.exit(1);
    }
    console.log(
      `✔ apps/home/content/generated/*.json and README.md is fresh (${Object.keys(files).length + 2} files).`,
    );
    return;
  }
  console.log(
    stale.length
      ? `✔ apps/home/content/generated/*.json and README.md written — updated: ${stale.join(", ")}.`
      : `✔ apps/home/content/generated/*.json and README.md written — nothing changed.`,
  );
}
