#!/usr/bin/env node
/**
 * check-charts-reuse.mjs — @qlik-coe-emea/qlabs-components-charts reuse-audit gate (#169).
 *
 * Enforces that `@qlik-coe-emea/qlabs-components-charts` does NOT define its own component with the same
 * name as a component already exported by `@qlik-coe-emea/qlabs-components-ui`. A name collision causes
 ...
 * charts must only import from allowed deps (no @base-ui bypass of @qlik-coe-emea/qlabs-components-ui).
 *
 ...
 *
 *   Class-1 (ui-name collision): flags a LOCAL runtime declaration whose name ∈ @qlik-coe-emea/qlabs-components-ui:
 ...
 *   Does NOT flag:
 *     import { X } from "@qlik-coe-emea/qlabs-components-ui"  — usage (pass-through is fine)
 *     export { X } from "@qlik-coe-emea/qlabs-components-ui"  — pass-through re-export
...
/**
 * Load @qlik-coe-emea/qlabs-components-ui component names from the manifest.
...
function loadUiNames() {
  ...
  const uiPkg = manifest?.packages?.["@qlik-coe-emea/qlabs-components-ui"];
  if (!uiPkg) {
    console.error(
      `✖ charts-reuse gate: manifest missing packages["@qlik-coe-emea/qlabs-components-ui"] — run \`pnpm manifest\`.`,
    );
    process.exit(1);
  }
 ...
 * @param {Set<string>} uiNames - set of @qlik-coe-emea/qlabs-components-ui component names to check against
...
export function findChartsReuseViolations(src, uiNames) {
  ...
  // ── Class 1: local runtime declarations whose name is in @qlik-coe-emea/qlabs-components-ui ───────
  //
...
function main(argv) {
  ...
  if (findings.length) {
    ...
    for (const v of findings) {
      ...
      if (v.kind === "collision") {
        console.error(
          `  - ${rel}: declares "${v.name}" which collides with @qlik-coe-emea/qlabs-components-ui\n` +
            `      ${v.statement}`,
        );
      } else {
        console.error(
    ...
    console.error(
      `\n@qlik-coe-emea/qlabs-components-charts must not define components whose names collide with @qlik-coe-emea/qlabs-components-ui exports,\n` +
  ...
  if (!warnOnly) {
    ...
    console.log(
      `✔ charts-reuse: no collisions with @qlik-coe-emea/qlabs-components-ui, no @base-ui imports (${scope}).`,
    );
  }
}
