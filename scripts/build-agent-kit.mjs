 ...
 *
 * The released `@elabs-ai/components-*` tarballs carry the components; this kit carries the
...
const SANITIZE = [
  ["packages/tokens/src/themes.css", "@elabs-ai/components-tokens/styles.css"],
  [
...
The coding-agent layer for the brand-ui design system, pinned to the \`@elabs-ai/components-*\`
...
Pairs with the \`@elabs-ai/components-*\` packages, published publicly on npmjs.org
— \`pnpm add @elabs-ai/components-ui\`, no registry configuration and no token. See
\`docs/CONSUMING.md\` in the repo.
...
It is a public npm package — nothing to configure first:

\`\`\`
pnpm add -D @elabs-ai/components-cli
pnpm exec brand-ui info            # themes, tokens, installed @elabs-ai/components-* packages
...
The CLI reads the manifest bundled inside \`@elabs-ai/components-cli\`, so \`search\`/\`docs\`
