 ...
 * research 10 §F).
 *
 * Issue #190 moved the `Timeline` rail from `@qlik-coe-emea/qlabs-components-editor` to `@qlik-coe-emea/qlabs-components-ui`
 ...
 *     (`Timeline`/`TimelineRoot`/`TimelineItem`) from `@qlik-coe-emea/qlabs-components-ui`.
 *
 * ALLOWLIST: empty since #192 — `chain-of-thought.tsx`'s legacy rail converged
 * onto `AgentTimeline` (built on the @qlik-coe-emea/qlabs-components-ui rail), so the gate now locks it
...
/** Reuse, not fork: importing the rail parts from @qlik-coe-emea/qlabs-components-ui exempts class 2. */
export const RAIL_IMPORT_RE =
  /import\s+(?:type\s+)?\{[^}]*\bTimeline(?:Root|Item)?\b[^}]*\}\s*from\s*["']@qlik-coe-emea\/qlabs-components-ui["']/;
...
function main(argv) {
  ...
  if (findings.length) {
    ...
    for (const v of findings) {
      ...
      if (v.kind === "name") {
        console.error(
          `  ${rel}:${v.line}  "${v.name}" is declared locally — the Timeline rail lives in @qlik-coe-emea/qlabs-components-ui`,
        );
      } else {
        console.error(
    ...
    console.error(
      `\nThe Timeline rail/node/connector spine lives ONCE, in\n` +
        ...
        `@qlik-coe-emea/qlabs-components-ui instead of re-declaring a Timeline* component or re-rolling the\n` +
