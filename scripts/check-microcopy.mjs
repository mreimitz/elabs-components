 ...
 *
 * `@elabs-ai/components-*` ships a locale seam (`LocaleProvider` + `t()`, ADR 0014/0017), but a
 * seam nothing calls is decoration: before ADR 0017, `t()` was invoked in ZERO
 * components while `@elabs-ai/components-ai` alone hardcoded ~100 user-visible strings —
