...
test("FLAGS: a consumedBy name not exported by @qlik-coe-emea/qlabs-components-ai", () => {
  const ao = makeAgentOutput();
  ao.paths.conversation.consumedBy = ["Conversation", "Nonexistent"];
  assert.ok(run(ao).some((p) => p.includes("Nonexistent")));
});

test("FLAGS: jsxPreview.component not exported by @qlik-coe-emea/qlabs-components-ai", () => {
  const ao = makeAgentOutput();
