...
function makeRepo({ pkg = "ui", name = "sample", story = STORY } = {}) {
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: `@elabs-ai/components-${pkg}` }),
  );
...
test("transform: strips Storybook scaffolding + rewrites same-package imports", () => {
  const { code, title, packages } = transformStory({
    src: STORY,
    pkgName: "@elabs-ai/components-ui",
    name: "sample",
  ...
  assert.ok(
    /import { Widget } from "@elabs-ai\/components-ui"/.test(code),
    "rewritten to @elabs-ai/components-ui",
  );
  assert.ok(
    /import { Button } from "@elabs-ai\/components-ui"/.test(code),
  ...
  assert.deepEqual(packages, ["@elabs-ai/components-ui"]);
});
