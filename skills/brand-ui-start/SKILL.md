...
One obvious entry point for working with **brand-ui** (`@qlik-coe-emea/qlabs-components-*`). Figure out
...
> - **Just help me use brand-ui** — the app already uses `@qlik-coe-emea/qlabs-components-*`; help me
...
1. Run the **deterministic profile** (read-only, edits nothing):
   ...
   first — `pnpm add -D @qlik-coe-emea/qlabs-components-cli`, see `docs/CONSUMING.md` §1 + §7a — then
...
`brand-ui` consumer skill and the `@qlik-coe-emea/qlabs-components-cli` commands (`brand-ui info` /
