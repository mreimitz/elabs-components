...
| ------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Build a new app           | `brand-ui-new-app` | Guided interview (archetype · **app shell** · theme · title) → `app-spec.md` → annotated scaffold from a template + playbook (VP-02). For an **enterprise/professional** surface, start with `brand-ui-enterprise` (classify register, pick the shell archetype + mandatory baseline), which then feeds this skill. |
| Improve an existing app   | `brand-ui-migrate` | Scan → map existing components to brand-ui → a phased plan → review-gated migration, one phase at a time; the app-shell phase asks which _Layout/App Shell_ the app moves into (never a hand-rolled frame). Analysis is read-only; the edits are made by the agent and approved by the person.                      |
