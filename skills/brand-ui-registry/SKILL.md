...
- **Package (`@elabs-ai/components-*` import)** — stable, reusable primitives many apps share.
...
1. Put source under `registry/` and add an entry to `registry/registry.json`:
   ...
   relative to repo root, must exist), `dependencies` (real npm + `@elabs-ai/components-*`),
   `registryDependencies` (other items), `target` for `registry:page`/`file`.
2. Keep imports on the `@elabs-ai/components-*` alias, semantic tokens only.
