...
via the shadcn CLI; stable primitives stay as imported `@elabs/components-*` packages.

## Decision: package vs registry

- **Package (`@elabs/components-*` import)** — stable, reusable primitives many apps share.
...
importing installed `@elabs/components-*` — list them in `dependencies`), and `registry:theme`
...
1. Put source under `registry/` and add an entry to `registry/registry.json`:
   ...
   relative to repo root, must exist), `dependencies` (real npm + `@elabs/components-*`),
   `registryDependencies` (other items), `target` for `registry:page`/`file`.
2. Keep imports on the `@elabs/components-*` alias, semantic tokens only.
