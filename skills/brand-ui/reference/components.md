...
| Images / audio / video | `Image` (fit, skeleton, fallback) · `Audio`, `Video` (own controls, docked or overlay bar, captions) · `MediaPlayer*` parts for a custom bar (`@elabs-ai/components-ui`). AI-generated image → `GeneratedImage`, spoken reply → `AudioPlayer*` (`@elabs-ai/components-ai`); a file the app did not write → `FileViewer` (`@elabs-ai/components-viewer`) |
...
| File / document viewer | `FileViewer` + `FileViewerProvider`/`FileViewerToolbar`/`FileViewerContent` (`@elabs-ai/components-viewer`) — images, audio, video, text, code, JSON, CSV, markdown, PDF, docx, pptx, xlsx; formats are added by registering an adapter; media renders through ui `Image`/`Audio`/`Video`                                                               |
