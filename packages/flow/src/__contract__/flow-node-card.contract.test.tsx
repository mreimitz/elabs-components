...
/**
 * Contract probe for FlowNodeCard (`packages/flow/src/flow-node-card/flow-node-card.tsx`), derived from its
 * `Default` story (packages/flow/src/flow-node-card/flow-node-card.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../flow-node-card/flow-node-card.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `flow-flownodecard--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("FlowNodeCard contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="flow-node-card" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="flow-node-card"]')).not.toBeNull();
  });
});
