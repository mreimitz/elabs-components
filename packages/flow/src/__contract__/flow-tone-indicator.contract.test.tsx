...
/**
 * Contract probe for FlowToneIndicator (`packages/flow/src/flow-tone/flow-tone-indicator.tsx`), derived from its
 * `Default` story (packages/flow/src/flow-tone/flow-tone-indicator.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../flow-tone/flow-tone-indicator.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `flow-flowtoneindicator--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("FlowToneIndicator contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="flow-tone-indicator" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="flow-tone-indicator"]')).not.toBeNull();
  });
});
