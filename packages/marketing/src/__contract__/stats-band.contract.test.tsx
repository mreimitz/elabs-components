...
/**
 * Contract probe for StatsBand (`packages/marketing/src/stats-band/stats-band.tsx`), derived from its
 * `Default` story (packages/marketing/src/stats-band/stats-band.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../stats-band/stats-band.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `marketing-statsband--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("StatsBand contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="stats-band" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="stats-band"]')).not.toBeNull();
  });
});
