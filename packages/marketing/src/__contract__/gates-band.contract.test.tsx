...
/**
 * Contract probe for GatesBand (`packages/marketing/src/gates-band/gates-band.tsx`), derived from its
 * `Default` story (packages/marketing/src/gates-band/gates-band.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../gates-band/gates-band.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `marketing-gatesband--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("GatesBand contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="gates-band" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="gates-band"]')).not.toBeNull();
  });
});
