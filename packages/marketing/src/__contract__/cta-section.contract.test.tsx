...
/**
 * Contract probe for CTASection (`packages/marketing/src/cta-section/cta-section.tsx`), derived from its
 * `Default` story (packages/marketing/src/cta-section/cta-section.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../cta-section/cta-section.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `marketing-ctasection--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("CTASection contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="cta-section" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="cta-section"]')).not.toBeNull();
  });
});
