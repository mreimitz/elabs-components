...
/**
 * Contract probe for UseCaseCard (`packages/marketing/src/use-case-card/use-case-card.tsx`), derived from its
 * `Default` story (packages/marketing/src/use-case-card/use-case-card.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../use-case-card/use-case-card.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `marketing-usecasecard--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("UseCaseCard contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="use-case-card" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="use-case-card"]')).not.toBeNull();
  });
});
