...
/**
 * Contract probe for LogoStrip (`packages/marketing/src/logo-strip/logo-strip.tsx`), derived from its
 * `Default` story (packages/marketing/src/logo-strip/logo-strip.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../logo-strip/logo-strip.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `marketing-logostrip--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("LogoStrip contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="logo-strip" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="logo-strip"]')).not.toBeNull();
  });
});
