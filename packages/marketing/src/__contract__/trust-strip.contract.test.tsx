...
/**
 * Contract probe for TrustStrip (`packages/marketing/src/trust-strip/trust-strip.tsx`), derived from its
 * `Default` story (packages/marketing/src/trust-strip/trust-strip.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../trust-strip/trust-strip.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `marketing-truststrip--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("TrustStrip contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="trust-strip" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="trust-strip"]')).not.toBeNull();
  });
});
