...
/**
 * Contract probe for FeatureGrid (`packages/marketing/src/feature-grid/feature-grid.tsx`), derived from its
 * `Default` story (packages/marketing/src/feature-grid/feature-grid.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../feature-grid/feature-grid.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `marketing-featuregrid--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("FeatureGrid contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="feature-grid" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="feature-grid"]')).not.toBeNull();
  });
});
