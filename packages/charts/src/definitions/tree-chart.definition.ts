/**
 * TreeChart definition (ADR 0042 §5, RM-176). Kind defaults match the destructuring of
 * `TreeChartBody` (`charts/tree-chart.tsx`); `nodeSize`/`nodeWidth`/`nodeHeight`'s defaults
 * are that module's own constants, written as their values because that module is not pure.
 * `zoomRange`, `defaultExpandedDepth` and `collapseDepth` have no kind default: each is left
 * bare in the destructuring (resolved, or left unset, deeper in the component).
 *
