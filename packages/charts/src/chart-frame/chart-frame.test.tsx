...
describe("ChartFrame density toolbar (#444)", () => {
  ...
  it("tile without height fills its host; an explicit (deprecated) height becomes the plot height", () => {
    ...
    // ADR 0039: `height` is an alias for `plotHeight` — it sizes the chart,
    // never the body, and the tile stops filling its host.
    const sized = container.querySelector('[data-slot="chart-frame-body"] > div') as HTMLElement;
    expect(sized.style.height).toBe("");
    expect(
      (container.querySelector('[data-slot="chart-frame"]') as HTMLElement).className,
