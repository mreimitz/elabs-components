...
/**
 * Contract probe for BulletChart (`packages/charts/src/charts/bullet-chart.tsx`), derived from its
 * `Default` story (packages/charts/src/charts/bullet-chart.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../charts/bullet-chart.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `charts-bulletchart--default|jsdom|jsdom|${assertion}`;
...
describe("BulletChart contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="bullet-chart" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="bullet-chart"]')).not.toBeNull();
  });
});
