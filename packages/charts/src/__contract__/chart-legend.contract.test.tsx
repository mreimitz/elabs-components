...
/**
 * Contract probe for ChartLegend (`packages/charts/src/charts/chart-legend.tsx`), derived from its
 * `Default` story (packages/charts/src/charts/chart-legend.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../charts/chart-legend.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `charts-chartlegend--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("ChartLegend contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="chart-legend" on its root', () => {
