...
export interface ChoroplethInteractionContextValue {
  ...
  /** Keyboard-focused feature index (roving tabindex). Null when no feature is keyboard-focused. */
  focusedFeatureIndex: number | null;
  setFocusedFeatureIndex: (index: number | null) => void;
...
export interface ChoroplethStableContextValue {
  ...
  // Raw path function for graticule (accepts any projectable GeoJSON object)
...
export function ChoroplethInteractionShell({ children }: { children: React.ReactNode }) {
  ...
  const [focusedFeatureIndex, setFocusedFeatureIndex] = useState<number | null>(null);
