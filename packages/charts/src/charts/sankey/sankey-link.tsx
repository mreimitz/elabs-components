...
export interface SankeyLinkProps {
  ...
  getLinkColor?: (link: SankeyLinkType<SankeyNodeDatum, SankeyLinkDatum>, index: number) => string;
...
function AnimatedLink({
  ...
  // `path` is the sole geometry input (`d={path}` below), so re-measuring is only
  // needed when it changes — without the dep array `getTotalLength()` ran on every
  // hover/fade re-render.
  useLayoutEffect(() => {
    if (pathRef.current) {
