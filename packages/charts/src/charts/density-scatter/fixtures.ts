/**
 * density-scatter/fixtures.ts — deterministic synthetic datasets for stories
 * and tests. Never `Math.random()` (charts-honesty): a hashed LCG seeded per
 * dataset, so a 200k-point story renders the same picture on every run.
 */

import type { DensityScatterColumns, DensityZone } from "./types";
...
/** A tube: a quadratic curve p0 → p1 bent by c, with a gaussian cross-section. */
interface Tube {
  p0: [number, number];
  c: [number, number];
  p1: [number, number];
...
/**
 * Lateral-deviation traffic along a track: a dense core that narrows past the
 * funnel throat, arrival arcs joining it, streaks beside it, a few outliers.
 * `x` = along-track distance (m), `y` = cross-track deviation (m),
 * `values.speed` = ground speed (kt) rising along the track.
 */
export function buildLateralTraffic(n = 200_000, seed = 20260923): DensityScatterColumns {
...
/** The operational-design-domain zones of `buildLateralTraffic`: core, expanded envelope. */
export const LATERAL_ZONES: readonly DensityZone[] = [
  {
    id: "core",
    label: "Core",
    color: "var(--chart-2)",
...
/**
 * Semiconductor wafer probe: die position (mm) on a 300 mm wafer with a
 * radial yield gradient and two defect clusters; `values.vth` = threshold
 * voltage (mV) drifting with radius; `categories.bin` = pass / marginal / fail.
 */
export function buildWaferProbe(n = 150_000, seed = 7): DensityScatterColumns {
  const random = lcg(seed);
  const gauss = gaussian(random);
  const x = new Float32Array(n);
  const y = new Float32Array(n);
  ...
  while (i < n) {
    ...
    if (r > 148) continue;
    // Edge dies are sparser (partial dies), centre denser.
    if (random() < r / 150 - 0.62) continue;
    x[i] = px;
    y[i] = py;
    let v = 420 + r * 0.55 + gauss() * 9;
    // Two defect clusters: a scratch and a hot spot.
...
/**
 * Order-book fills: order size (log10 USD) against fill latency (ms) for one
 * trading day — a dense retail cloud, a block-trade tail, a slow-venue band.
 * `values.slippage` = basis points.
 */
export function buildFillLatency(n = 250_000, seed = 99): DensityScatterColumns {
  const random = lcg(seed);
  const gauss = gaussian(random);
  const x = new Float32Array(n);
  const y = new Float32Array(n);
