...
const LiveYAxisInner = memo(function LiveYAxisInner({
  ...
  const stableTickValues = useMemo(() => {
    ...
    return values;
    // eslint-disable-next-line react-hooks/exhaustive-deps -- quantizedMin/quantizedMax are intentional coarse-grained stability deps
      ...
      >
        <AnimatePresence initial={false}>
          {tickData.map((tick) => (
            ...
            >
              <span className="whitespace-nowrap font-mono text-chart-label text-meta">
                {tick.label}
              </span>
            </motion.div>
          ))}
