...
/** Every colour-bearing `var(--…)` reference the render paints, sorted and de-duplicated. */
function colorsIn(container: HTMLElement): string[] {
  ...
  for (const el of Array.from(container.querySelectorAll("*"))) {
    for (const attr of Array.from(el.attributes)) {
      ...
      for (const match of attr.value.match(VAR_RE) ?? []) found.add(match);
    }
  }
  return [...found].sort();
}
...
/**
 * The colour references each family drew with no `palette`, recorded from the pre-RM-186
 * code (744f17a2) with this harness. RM-186 moves no default, so this never changes with it.
 */
const DEFAULT_COLORS: Readonly<Record<string, readonly string[]>> = {
  AreaChart: ["var(--chart-line-primary)"],
  BarChart: ["var(--chart-1)", "var(--chart-2)", "var(--ease-standard)", "var(--t-fast)"],
  BulletChart: ["var(--chart-1)", "var(--chart-foreground)", "var(--chart-ring-background)"],
  BumpChart: ["var(--chart-1)", "var(--chart-2)", "var(--chart-background)", "var(--chart-label)"],
  CandlestickChart: ["var(--chart-1)", "var(--chart-5)"],
  ChoroplethChart: ["var(--background)", "var(--chart-1)", "var(--ease-entrance)", "var(--t-fast)"],
  ComposedChart: ["var(--chart-line-primary)"],
  DensityScatterChart: ["var(--chart-1)", "var(--chart-2)", "var(--chart-3)", "var(--chart-grid)"],
  DistributionChart: [
    "var(--chart-1)",
  ...
  LineChart: ["var(--chart-line-primary)"],
  LiveLineChart: [
    "var(--chart-background)",
  ...
  PieChart: ["var(--chart-1)", "var(--chart-2)", "var(--chart-3)"],
  RadarChart: [
    "var(--chart-1)",
    "var(--chart-background)",
    "var(--chart-foreground-muted)",
  ...
  RingChart: ["var(--chart-1)", "var(--chart-2)", "var(--chart-ring-background)"],
  SankeyChart: [
    "var(--chart-1)",
    "var(--chart-2)",
    "var(--chart-3)",
  ...
  UnitChart: ["var(--chart-1)", "var(--chart-2)"],
  WaterfallChart: [
    "var(--chart-2)",
    "var(--chart-background)",
    "var(--chart-foreground)",
