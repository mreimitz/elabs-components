...
function MarkerCircleHTML({
  ...
  return (
    <motion.div
      className={cn(
        "relative flex h-full w-full items-center justify-center rounded-full shadow-sm",
        // The hover lift is a RUNG CHANGE, not a hand-rolled box-shadow: a raw
        // `whileHover={{ boxShadow: "0 4px 20px rgba(0,0,0,.25)" }}` was both a
        // raw color and outside the elevation ramp (so it ignored the theme's
        // shadow ink and the shadowless dial). motion drives the scale; the
        // shadow steps sm → md on the class, gated by the motion tokens.
        hasAction && "cursor-pointer transition-shadow duration-fast ease-standard hover:shadow-md",
      )}
      ...
      whileHover={hasAction ? { scale: 1.15 } : undefined}
