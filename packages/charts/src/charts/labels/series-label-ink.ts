 ...
 * in OKLab, which keeps the series hue (an OKLCH mix rotates it toward the
 * label ink's hue). The line, swatch and marker keep the pure stroke.
 */
 ...
 * every built-in theme, measured in the browser with the OKLab mix
 * (2026-09-18). The binding case is light `--chart-10`.
 */
...
export function seriesLabelInk(stroke: string): string {
  ...
  return `color-mix(in oklab, ${stroke} ${SERIES_LABEL_INK_MIX}%, var(--chart-label))`;
}
