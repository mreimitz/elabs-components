 ...
 * regenerated by `pnpm gen`), then the copy bundled with the CLI.
 *
  ...
  if (imports === null) {
    return {
      error:
        ...
        `templates bundled with the CLI). Run from a brand-ui checkout (\`pnpm gen\` ` +
...
export function planScaffold(spec, { root, bundledDir } = {}) {
  ...
  return {
    ...
    // The agent handoff (#123 AC2): the two contracts + a short, manifest-derived
    // list of the installed packages' components so a later session's agent
...
/**
 * `brand-ui-context.md` — the routine plus one name per component for the
 * packages this app installs (#123 AC2: a context file next to CLAUDE.md /
 * AGENTS.md, so a later agent session knows what exists without a running
 * Storybook, an MCP server, or a guess). Short on purpose: the agent reads it
 * before its first edit, and `brand-ui docs <Name> --brief` has the rest.
 */
function buildContextFile(root, packages) {
  const manifest = loadManifest(root);
  if (manifest) return renderAppContext(manifest, packages);
  return `# brand-ui in this app

The component manifest was not reachable when this app was created, so there is
no component list here. \`brand-ui search <concept>\` and \`brand-ui docs <Name> --brief\`
answer once the app's packages are installed (\`pnpm install\` or \`npm install\`);
run them as \`pnpm exec brand-ui …\` or \`npx brand-ui …\`.
`;
}
...
With npm, run the same two lines with \`npm install\` in place of \`pnpm add\`.
...
- **Use brand-ui components first.** Before writing any UI markup, check
  ...
  component (\`pnpm exec brand-ui search <concept>\`, with npm
  \`npx brand-ui search <concept>\`, or the \`mcp__brand-ui__search\` tool in Claude
...
\`./brand-ui-context.md\` lists the components of the packages this app installs,
one name each, and the routine — read it before inventing a component. For one
component's parts and real props: \`pnpm exec brand-ui docs <Name> --brief\` (with
npm \`npx brand-ui docs <Name> --brief\`), or the \`mcp__brand-ui__docs\` tool. Drop
\`--brief\` for every prop. \`search\` covers every package, installed or not.
...
With npm: \`npm run dev\`, \`npm run typecheck\`, \`npm run lint\`, \`npm run audit:ui\`.
...
- \`./brand-ui-context.md\` lists the components of the packages this app installs —
  read it instead of inventing one; \`brand-ui docs <Name> --brief\` gives the parts
  and real props.
...
export function emitScaffold(
  ...
  if (!templateAbs) {
    return {
      ...base,
      status: "error",
      error: `template not found: ${templatePath(archetype)} (run \`pnpm gen\`, or run from inside the monorepo)`,
    };
  }
  ...
  const files = {
    ...
    "brand-ui-context.md": buildContextFile(root, packages),
