/**
 * @qlik-coe-emea/qlabs-components-cli — persistent `brand-ui mcp` stdio server (WP-03 #81).
 *
 ...
 * 2.0, which is small enough to implement directly. That keeps `@qlik-coe-emea/qlabs-components-cli`
 * dependency-light (it runs in any consuming project via `npx @qlik-coe-emea/qlabs-components-cli mcp`) and
...
export const TOOLS = [
  {
    name: "info",
    description:
      "Project context: which @qlik-coe-emea/qlabs-components-* packages are present, the themes + default, the token count, the registry size, and the ACTIVE taste profile (register × density × motion × expressiveness). Call once at the start of a session.",
...
function toolInfo(root) {
  ...
  if (!manifest)
    return textContent(
      "No manifest — run inside the brand-ui monorepo or install @qlik-coe-emea/qlabs-components-cli.",
    );
