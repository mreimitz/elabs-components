/**
 * @qlik-coe-emea/qlabs-components-cli — shared, deterministic renderers for the generated agent docs.
 *
...
export const INFRA_PKGS = {
  ...
  "@qlik-coe-emea/qlabs-components-docs": { path: "apps/docs", purpose: "Storybook" },
...
export function renderInventory(manifest) {
  ...
  lines.push("");
  lines.push(
    "_Generated by `@qlik-coe-emea/qlabs-components-cli`. The live, queryable surface is `brand-ui docs <Component>` " +
...
export function renderLlmsHub(manifest) {
  ...
  lines.push(
    "This file is generated from `brand-ui.manifest.json` — the anti-hallucination ground " +
      ...
      "dependency from GitHub Packages (`pnpm add -D @qlik-coe-emea/qlabs-components-cli`, see " +
  ...
  for (const pkg of orderedPackages(manifest)) {
    ...
    const hooks = (info.hooks || []).length;
    lines.push(
      `- [${pkg}](./llms/${pkg.replace("@qlik-coe-emea/qlabs-components-", "")}.txt) — ${PKG_PURPOSE[pkg] || ""} ` +
  ...
  lines.push("");
  lines.push(
    "- CLI: install first — `pnpm add -D @qlik-coe-emea/qlabs-components-cli` (GitHub Packages, " +
...
export function renderLlmsSpoke(manifest, pkgName) {
  ...
  // The @qlik-coe-emea/qlabs-components-ai OUTPUT CONTRACT: the shape an agent must emit so these
  ...
  if (pkgName === "@qlik-coe-emea/qlabs-components-ai" && manifest.agentOutput?.paths) {
 ...
 *           "list"    → a bulleted list (`- **@qlik-coe-emea/qlabs-components-x** — purpose.`) for Introduction.mdx
 ...
 * components", "13 charts", "qlik-bright/qlik-dark themes"); it is now generated
...
// ---------------------------------------------------------------------------
// AGENT-OUTPUT CONTRACT — "how an agent structures output so @qlik-coe-emea/qlabs-components-ai renders it"
...
export function renderAgentOutputGuidance(manifest) {
  ...
  lines.push(
    "`@qlik-coe-emea/qlabs-components-ai` is a **presentation layer**: it renders a data model — your app owns the " +
  ...
  if (tool) {
    lines.push(
      `A **tool part** is typed \`${tool.typePattern}\` and carries ` +
        ...
        "closed `@qlik-coe-emea/qlabs-components-ui` `Status` enum:",
    );
    lines.push("");
