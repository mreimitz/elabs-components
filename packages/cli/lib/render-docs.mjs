/**
 * @elabs/components-cli — shared, deterministic renderers for the generated agent docs.
 *
...
export const INFRA_PKGS = {
  ...
  "@elabs/components-docs": { path: "apps/docs", purpose: "Storybook" },
...
export function renderInventory(manifest) {
  ...
  lines.push("");
  lines.push(
    "_Generated by `@elabs/components-cli`. The live, queryable surface is `brand-ui docs <Component>` " +
...
export function renderLlmsHub(manifest) {
  ...
  lines.push(
    "This file is generated from `brand-ui.manifest.json` — the anti-hallucination ground " +
      ...
      "dependency from GitHub Packages (`pnpm add -D @elabs/components-cli`, see " +
  ...
  for (const pkg of orderedPackages(manifest)) {
    ...
    const hooks = (info.hooks || []).length;
    lines.push(
      `- [${pkg}](./llms/${pkg.replace("@elabs/components-", "")}.txt) — ${PKG_PURPOSE[pkg] || ""} ` +
  ...
  lines.push("");
  lines.push(
    "- CLI: install first — `pnpm add -D @elabs/components-cli` (GitHub Packages, " +
...
export function renderLlmsSpoke(manifest, pkgName) {
  ...
  // The @elabs/components-ai OUTPUT CONTRACT: the shape an agent must emit so these
  ...
  if (pkgName === "@elabs/components-ai" && manifest.agentOutput?.paths) {
 ...
 *           "list"    → a bulleted list (`- **@elabs/components-x** — purpose.`) for Introduction.mdx
...
// ---------------------------------------------------------------------------
// AGENT-OUTPUT CONTRACT — "how an agent structures output so @elabs/components-ai renders it"
...
export function renderAgentOutputGuidance(manifest) {
  ...
  lines.push(
    "`@elabs/components-ai` is a **presentation layer**: it renders a data model — your app owns the " +
  ...
  if (tool) {
    lines.push(
      `A **tool part** is typed \`${tool.typePattern}\` and carries ` +
        ...
        "closed `@elabs/components-ui` `Status` enum:",
    );
    lines.push("");
