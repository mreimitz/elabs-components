...
const FIXTURE = {
  ...
  packages: {
    ...
    "@elabs/components-editor": {
      ...
      otherExports: [],
      subpaths: {
        "@elabs/components-editor/markdown/frontmatter": {
          components: [],
          hooks: [
...
test("orderedPackages puts known packages in dependency-ish order", () => {
  ...
  assert.deepEqual(order, ["@elabs/components-ui", "@elabs/components-editor"]);
});

test("packageRows sorts, summarizes variants, and surfaces subpath imports", () => {
  const rows = packageRows(FIXTURE, "@elabs/components-ui");
  assert.deepEqual(
    rows.map((r) => r.name),
    ["Button", "Card", "useFoo"],
  ...
  const editorRows = packageRows(FIXTURE, "@elabs/components-editor");
  const sub = editorRows.find((r) => r.name === "useFrontmatter");
  assert.equal(sub.importPath, "@elabs/components-editor/markdown/frontmatter");
});
...
test("renderInventory is deterministic and lists every package + component", () => {
  ...
  assert.match(a, /## @elabs\/components-ui/);
  assert.match(a, /Button/);
  assert.match(a, /CodeEditor/);
  assert.match(a, /\| `@elabs\/components-ui` \| packages\/ui \| 2 \| 1 \|/, "package summary row");
...
test("renderLlmsHub routes to per-package spokes and lists themes + entry points", () => {
  ...
  assert.match(hub, /\[@elabs\/components-ui\]\(\.\/llms\/ui\.txt\)/, "spoke link");
  assert.match(hub, /light \(default\)/);
  assert.match(hub, /pnpm add -D @elabs\/components-cli/);
...
test("renderLlmsSpoke shows a package's components, variants and anti-patterns", () => {
  const spoke = renderLlmsSpoke(FIXTURE, "@elabs/components-ui");
  assert.match(spoke, /# @elabs\/components-ui/);
  assert.match(
    spoke,
...
test("renderLlmsSpoke lists subpath exports separately", () => {
  const spoke = renderLlmsSpoke(FIXTURE, "@elabs/components-editor");
  assert.match(spoke, /## Subpath exports/);
  assert.match(spoke, /useFrontmatter \(`@elabs\/components-editor\/markdown\/frontmatter`/);
});

test("renderContextBlock is a concise names-only catalogue (no prop tables)", () => {
  const ctx = renderContextBlock(FIXTURE);
  assert.match(ctx, /### @elabs\/components-ui/);
...
test("renderers tolerate a manifest with empty/missing buckets", () => {
  const empty = {
    themes: [],
    packages: { "@elabs/components-ui": { path: "packages/ui", components: [] } },
  };
  ...
  assert.doesNotThrow(() => renderLlmsSpoke(empty, "@elabs/components-ui"));
});
