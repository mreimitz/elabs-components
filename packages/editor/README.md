...
@source "../node_modules/@elabs-ai/components-editor/dist";
```
...
This mirrors how `@elabs-ai/components-flow` wraps React Flow and `@elabs-ai/components-data` wraps TanStack:
a powerful third-party engine behind a token-driven brand API. It is intentionally
**not** "source-owned" the way primitives in `@elabs-ai/components-ui` are.
...
`@elabs-ai/components-ai` (Shiki) instead.
