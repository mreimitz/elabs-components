...
/**
 * Contract probe for EmptyState (`packages/ui/src/components/empty-state/empty-state.tsx`), derived from its
 * `Default` story (packages/ui/src/components/empty-state/empty-state.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/empty-state/empty-state.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `states-emptystate--default|jsdom|jsdom|${assertion}`;
...
describe("EmptyState contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="empty-state" on its root', () => {
