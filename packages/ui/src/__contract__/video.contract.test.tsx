...
/**
 * Contract probe for Video (`packages/ui/src/components/media-player/video.tsx`), derived from its
 * `Default` story (packages/ui/src/components/media-player/video.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/media-player/video.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `display-video--default|jsdom|jsdom|${assertion}`;
...
describe("Video contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="video" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="video"]')).not.toBeNull();
  });
});
