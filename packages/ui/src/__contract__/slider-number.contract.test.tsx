...
/**
 * Contract probe for SliderNumber (`packages/ui/src/components/slider-number/slider-number.tsx`), derived from its
 * `Default` story (packages/ui/src/components/slider-number/slider-number.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/slider-number/slider-number.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `forms-slidernumber--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("SliderNumber contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="slider-number" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="slider-number"]')).not.toBeNull();
  });
});
