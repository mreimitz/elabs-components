...
/**
 * Contract probe for CommandChip (`packages/ui/src/components/command-chip/command-chip.tsx`), derived from its
 * `Default` story (packages/ui/src/components/command-chip/command-chip.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/command-chip/command-chip.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `display-commandchip--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("CommandChip contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="command-chip" on its root', () => {
