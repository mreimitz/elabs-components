...
/**
 * Contract probe for AttributionPanel (`packages/ui/src/components/attribution-panel/attribution-panel.tsx`), derived from its
 * `Default` story (packages/ui/src/components/attribution-panel/attribution-panel.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/attribution-panel/attribution-panel.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `core-attributionpanel--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("AttributionPanel contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="attribution-panel" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="attribution-panel"]')).not.toBeNull();
  });
});
