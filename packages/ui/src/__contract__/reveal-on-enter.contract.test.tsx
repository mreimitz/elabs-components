...
/**
 * Contract probe for RevealOnEnter (`packages/ui/src/components/reveal-on-enter/reveal-on-enter.tsx`), derived from its
 * `Default` story (packages/ui/src/components/reveal-on-enter/reveal-on-enter.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/reveal-on-enter/reveal-on-enter.stories";
...
describe("RevealOnEnter contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="reveal-on-enter" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="reveal-on-enter"]')).not.toBeNull();
  });
});
