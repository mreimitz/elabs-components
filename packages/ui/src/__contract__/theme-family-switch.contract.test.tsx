...
/**
 * Contract probe for ThemeFamilySwitch (`packages/ui/src/components/theme-family-switch/theme-family-switch.tsx`), derived from its
 * `Default` story (packages/ui/src/components/theme-family-switch/theme-family-switch.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/theme-family-switch/theme-family-switch.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `core-themefamilyswitch--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("ThemeFamilySwitch contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="theme-family-switch" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="theme-family-switch"]')).not.toBeNull();
  });
});
