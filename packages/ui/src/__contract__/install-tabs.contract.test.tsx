...
/**
 * Contract probe for InstallTabs (`packages/ui/src/components/install-tabs/install-tabs.tsx`), derived from its
 * `Default` story (packages/ui/src/components/install-tabs/install-tabs.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/install-tabs/install-tabs.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `display-installtabs--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("InstallTabs contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="install-tabs" on its root', () => {
