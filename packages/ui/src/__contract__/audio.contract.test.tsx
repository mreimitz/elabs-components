...
/**
 * Contract probe for Audio (`packages/ui/src/components/media-player/audio.tsx`), derived from its
 * `Default` story (packages/ui/src/components/media-player/audio.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/media-player/audio.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `display-audio--default|jsdom|jsdom|${assertion}`;
...
describe("Audio contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="audio" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="audio"]')).not.toBeNull();
  });
});
