...
/**
 * Contract probe for TreeSelect (`packages/ui/src/components/tree-select/tree-select.tsx`), derived from its
 * `Default` story (packages/ui/src/components/tree-select/tree-select.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/tree-select/tree-select.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `forms-treeselect--default|jsdom|jsdom|${assertion}`;
...
describe("TreeSelect contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="tree-select" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="tree-select"]')).not.toBeNull();
  });
});
