...
/**
 * Contract probe for ColorPicker (`packages/ui/src/components/color-picker/color-picker.tsx`), derived from its
 * `Default` story (packages/ui/src/components/color-picker/color-picker.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/color-picker/color-picker.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `forms-colorpicker--default|jsdom|jsdom|${assertion}`;
...
describe("ColorPicker contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="color-picker" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="color-picker"]')).not.toBeNull();
  });
});
