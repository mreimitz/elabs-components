...
/**
 * Contract probe for IntegrationMatrix (`packages/ui/src/components/integration-matrix/integration-matrix.tsx`), derived from its
 * `Default` story (packages/ui/src/components/integration-matrix/integration-matrix.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/integration-matrix/integration-matrix.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `display-integrationmatrix--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("IntegrationMatrix contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="integration-matrix" on its root', () => {
