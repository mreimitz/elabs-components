...
/**
 * Contract probe for SegmentedField (`packages/ui/src/components/segmented-field/segmented-field.tsx`), derived from its
 * `Default` story (packages/ui/src/components/segmented-field/segmented-field.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/segmented-field/segmented-field.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `forms-segmentedfield--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("SegmentedField contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="segmented-field" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="segmented-field"]')).not.toBeNull();
  });
});
