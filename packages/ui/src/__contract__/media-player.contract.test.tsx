...
/**
 * Contract probe for MediaPlayer (`packages/ui/src/components/media-player/media-player.tsx`), derived from its
 * `Default` story (packages/ui/src/components/media-player/media-player.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/media-player/media-player.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `display-mediaplayer--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("MediaPlayer contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="media-player" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="media-player"]')).not.toBeNull();
  });
});
