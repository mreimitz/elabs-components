...
/**
 * Contract probe for TokenSpotlight (`packages/ui/src/components/token-spotlight/token-spotlight.tsx`), derived from its
 * `Default` story (packages/ui/src/components/token-spotlight/token-spotlight.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/token-spotlight/token-spotlight.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `display-tokenspotlight--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("TokenSpotlight contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="token-spotlight" on its root', () => {
