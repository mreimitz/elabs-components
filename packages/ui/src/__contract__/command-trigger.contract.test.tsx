...
/**
 * Contract probe for CommandTrigger (`packages/ui/src/components/command-trigger/command-trigger.tsx`), derived from its
 * `Default` story (packages/ui/src/components/command-trigger/command-trigger.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/command-trigger/command-trigger.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `navigation-commandtrigger--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("CommandTrigger contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="command-trigger" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="command-trigger"]')).not.toBeNull();
  });
});
