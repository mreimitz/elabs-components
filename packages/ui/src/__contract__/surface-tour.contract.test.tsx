...
/**
 * Contract probe for SurfaceTour (`packages/ui/src/components/surface-tour/surface-tour.tsx`), derived from its
 * `Default` story (packages/ui/src/components/surface-tour/surface-tour.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/surface-tour/surface-tour.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `layout-surfacetour--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("SurfaceTour contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="surface-tour" on its root', () => {
