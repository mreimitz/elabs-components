...
/**
 * Contract probe for AmbientField (`packages/ui/src/components/ambient-field/ambient-field.tsx`), derived from its
 * `Default` story (packages/ui/src/components/ambient-field/ambient-field.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/ambient-field/ambient-field.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `display-ambientfield--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("AmbientField contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="ambient-field" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="ambient-field"]')).not.toBeNull();
  });
});
