...
/**
 * Contract probe for CopyableValue (`packages/ui/src/components/copyable-value/copyable-value.tsx`), derived from its
 * `Default` story (packages/ui/src/components/copyable-value/copyable-value.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/copyable-value/copyable-value.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `display-copyablevalue--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("CopyableValue contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="copyable-value" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="copyable-value"]')).not.toBeNull();
  });
});
