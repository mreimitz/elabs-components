...
/**
 * Contract probe for DateRangePicker (`packages/ui/src/components/date-range-picker/date-range-picker.tsx`), derived from its
 * `Default` story (packages/ui/src/components/date-range-picker/date-range-picker.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/date-range-picker/date-range-picker.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `forms-daterangepicker--default|jsdom|jsdom|${assertion}`;
...
describe("DateRangePicker contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="date-range-picker" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="date-range-picker"]')).not.toBeNull();
  });
});
