...
/**
 * Contract probe for SpecPlayground (`packages/ui/src/components/spec-playground/spec-playground.tsx`), derived from its
 * `Default` story (packages/ui/src/components/spec-playground/spec-playground.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/spec-playground/spec-playground.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `launch-checklist--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("SpecPlayground contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="spec-playground" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="spec-playground"]')).not.toBeNull();
  });
});
