...
/**
 * Contract probe for Combobox (`packages/ui/src/components/combobox/combobox.tsx`), derived from its
 * `Default` story (packages/ui/src/components/combobox/combobox.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/combobox/combobox.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `forms-combobox--default|jsdom|jsdom|${assertion}`;
...
describe("Combobox contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="combobox" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="combobox"]')).not.toBeNull();
  });
});
