...
/**
 * Contract probe for BoundedNumber (`packages/ui/src/components/bounded-number/bounded-number.tsx`), derived from its
 * `Default` story (packages/ui/src/components/bounded-number/bounded-number.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/bounded-number/bounded-number.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `forms-boundednumber--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("BoundedNumber contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="bounded-number" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="bounded-number"]')).not.toBeNull();
  });
});
