...
/**
 * Contract probe for VirtualSelect (`packages/ui/src/components/virtual-select/virtual-select.tsx`), derived from its
 * `Default` story (packages/ui/src/components/virtual-select/virtual-select.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../components/virtual-select/virtual-select.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `forms-virtualselect--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("VirtualSelect contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="virtual-select" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="virtual-select"]')).not.toBeNull();
  });
});
