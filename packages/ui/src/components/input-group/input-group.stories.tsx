...
const meta = {
  ...
  argTypes: {
    variant: {
      description:
        "`outline` — bordered form-field look (default). `surface` — soft filled composer look with no hard border. `card` — `bg-card` well for nesting inside an already-tinted outer frame (the double card, #254); its fill is theme-driven, not universally white — raised on light themes, recessed on qlik-dark.",
      control: { type: "radio" },
...
// the focus-within ring — no hard border. Used by `PromptInput` in @qlik-coe-emea/qlabs-components-ai.
...
// themes, recessed on qlik-dark/blueprint), not universally white.
