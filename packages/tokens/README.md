...
```bash
pnpm add @qlik-coe-emea/qlabs-components-tokens
```
...
@source "../node_modules/@qlik-coe-emea/qlabs-components-tokens/dist";
```
...
18 exported components — including `DECORATION_LEVELS`, `DecorationProvider`, `DEFAULT_DECORATION_LEVEL`, `DEFAULT_DENSITY`, `DEFAULT_MOTION_PREFERENCE`.
...
([`docs/ADR/0027-focus-ring-token-contract.md`](https://github.com/Qlik-CoE-EMEA/qlabs-components/blob/main/docs/ADR/0027-focus-ring-token-contract.md),
...
```css
/* Your app's CSS, after importing @qlik-coe-emea/qlabs-components-tokens/styles.css */
...
```bash
pnpm roles:check
pnpm --filter @qlik-coe-emea/qlabs-components-tokens test
```

`--sidebar-ring: var(--ring)` mirrors your override into the sidebar
automatically — nothing else to change there.

**Prefer forking the theme with [`/new-theme`](https://github.com/Qlik-CoE-EMEA/qlabs-components/blob/main/.claude/commands/new-theme.md)
