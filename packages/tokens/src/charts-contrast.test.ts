 ...
 *     (≥ 4.5:1), and so does `--chart-foreground-muted` (#183 — every shipped
 *     call site renders prose with it, so it is gated on the text rung even
 *     though furniture uses the same token below its own 3:1 need).
 */
...
describe("themes.css — chart palette WCAG contrast (all themes)", () => {
  describe.each(THEMES)("%s", (theme) => {
    ...
    // #183 — gated at AA_TEXT (4.5:1), not the NON_TEXT (3:1) mark bar: every
    // shipped call site (`Marginalia`'s note, the chart source-row caption)
    // renders sentence-length prose in this token, which wants the text rung.
    // Both reference themes already clear it (6.00 light / 6.45 dark) — this
    // is a gate change, not a
    // retune, closing the latent regression path a future `--muted-foreground`
    // tune (still passing its own 3:1 gate) would otherwise open silently.
    it("--chart-foreground-muted ≥ 4.5:1 on --chart-background", () => {
      const ratio = contrast(
