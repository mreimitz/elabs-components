/**
 * theme-decoration-parity.test.ts — `THEME_META[…].decorationLevel` must equal the
 * `--decoration` the theme's own block sets in themes.css.
 *
 * Why this needs teeth: the CSS is what actually RENDERS the dial, but
 * `ThemeProvider` derives `effectiveDecoration` from `THEME_META`
 * (`decoration ?? THEME_META[theme].decorationLevel ?? 0`). While nothing read that
...
import { THEMES, THEME_META, type ThemeName } from "./theme-types";
...
describe("theme decoration parity (themes.css ⇄ THEME_META)", () => {
  it.each(THEMES)("%s declares the same dial in CSS and in THEME_META", (theme) => {
    expect(THEME_META[theme].decorationLevel ?? 0).toBe(cssDecorationLevel(theme));
  });
  ...
  it("every declared level is a valid dial position", () => {
    for (const theme of THEMES) {
      const level = THEME_META[theme].decorationLevel ?? 0;
