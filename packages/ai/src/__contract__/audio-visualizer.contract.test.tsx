...
/**
 * Contract probe for AudioVisualizer (`packages/ai/src/audio-visualizer.tsx`), derived from its
 * `Default` story (packages/ai/src/audio-visualizer.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../audio-visualizer.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `ai-audiovisualizer--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("AudioVisualizer contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="audio-visualizer" on its root', () => {
