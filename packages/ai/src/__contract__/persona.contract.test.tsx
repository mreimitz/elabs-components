...
/**
 * Contract probe for Persona (`packages/ai/src/persona.tsx`), derived from its
 * `Default` story (packages/ai/src/persona.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `ai-persona--default|jsdom|jsdom|${assertion}`;
...
describe("Persona contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="persona" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="persona"]')).not.toBeNull();
  });
});
