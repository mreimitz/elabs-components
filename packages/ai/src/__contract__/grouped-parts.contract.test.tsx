...
/**
 * Contract probe for GroupedParts (`packages/ai/src/grouped-parts.tsx`), derived from its
 * `Default` story (packages/ai/src/grouped-parts.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../grouped-parts.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `ai-groupedparts--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("GroupedParts contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="grouped-parts" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="grouped-parts"]')).not.toBeNull();
  });
});
