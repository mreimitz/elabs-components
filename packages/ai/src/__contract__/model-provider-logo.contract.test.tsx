...
/**
 * Contract probe for ModelProviderLogo (`packages/ai/src/model-provider-logo.tsx`), derived from its
 * `Default` story (packages/ai/src/model-provider-logo.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../model-provider-logo.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `ai-modelproviderlogo--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("ModelProviderLogo contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="model-provider-logo" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="model-provider-logo"]')).not.toBeNull();
  });
});
