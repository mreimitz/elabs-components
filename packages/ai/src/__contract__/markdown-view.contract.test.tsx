...
/**
 * Contract probe for MarkdownView (`packages/ai/src/markdown-view.tsx`), derived from its
 * `Default` story (packages/ai/src/markdown-view.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../markdown-view.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `ai-markdownview--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("MarkdownView contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="markdown-view" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="markdown-view"]')).not.toBeNull();
  });
});
