...
/**
 * Contract probe for PromptInputEffort (`packages/ai/src/prompt-input-effort.tsx`), derived from its
 * `Default` story (packages/ai/src/prompt-input-effort.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../prompt-input-effort.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `ai-composer-promptinputeffort--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("PromptInputEffort contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="prompt-input-effort" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="prompt-input-effort"]')).not.toBeNull();
  });
});
