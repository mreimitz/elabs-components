...
/**
 * Contract probe for MessageFeedback (`packages/ai/src/message-feedback.tsx`), derived from its
 * `Default` story (packages/ai/src/message-feedback.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../message-feedback.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `ai-messagefeedback--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("MessageFeedback contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="message-feedback" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="message-feedback"]')).not.toBeNull();
  });
});
