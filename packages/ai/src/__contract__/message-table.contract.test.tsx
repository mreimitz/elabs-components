...
/**
 * Contract probe for MessageTable (`packages/ai/src/message-table.tsx`), derived from its
 * `Default` story (packages/ai/src/message-table.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../message-table.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `ai-messagetable--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("MessageTable contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="message-table" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="message-table"]')).not.toBeNull();
  });
});
