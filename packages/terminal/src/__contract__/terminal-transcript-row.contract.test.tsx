...
/**
 * Contract probe for TerminalTranscriptRow (`packages/terminal/src/terminal-transcript-row.tsx`), derived from its
 * `Default` story (packages/terminal/src/terminal-transcript-row.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../terminal-transcript-row.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `terminal-terminaltranscriptrow--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("TerminalTranscriptRow contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="terminal-transcript-row" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="terminal-transcript-row"]')).not.toBeNull();
  });
});
