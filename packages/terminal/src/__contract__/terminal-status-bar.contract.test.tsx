...
/**
 * Contract probe for TerminalStatusBar (`packages/terminal/src/terminal-status-bar.tsx`), derived from its
 * `Default` story (packages/terminal/src/terminal-status-bar.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../terminal-status-bar.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `terminal-terminalstatusbar--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("TerminalStatusBar contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="terminal-status-bar" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="terminal-status-bar"]')).not.toBeNull();
  });
});
