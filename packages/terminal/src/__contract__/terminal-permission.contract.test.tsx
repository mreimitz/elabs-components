...
/**
 * Contract probe for TerminalPermission (`packages/terminal/src/terminal-permission.tsx`), derived from its
 * `Default` story (packages/terminal/src/terminal-permission.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../terminal-permission.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `terminal-terminalpermission--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("TerminalPermission contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="terminal-permission" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="terminal-permission"]')).not.toBeNull();
  });
});
