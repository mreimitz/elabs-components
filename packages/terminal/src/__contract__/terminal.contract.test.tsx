...
/**
 * Contract probe for Terminal (`packages/terminal/src/terminal.tsx`), derived from its
 * `Default` story (packages/terminal/src/terminal.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `terminal-terminal--default|jsdom|jsdom|${assertion}`;
...
describe("Terminal contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="terminal" on its root', () => {
