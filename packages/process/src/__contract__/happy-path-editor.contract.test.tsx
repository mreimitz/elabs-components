...
/**
 * Contract probe for HappyPathEditor (`packages/process/src/happy-path-editor/happy-path-editor.tsx`), derived from its
 * `Default` story (packages/process/src/happy-path-editor/happy-path-editor.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../happy-path-editor/happy-path-editor.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `process-happypatheditor--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("HappyPathEditor contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="happy-path-editor" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="happy-path-editor"]')).not.toBeNull();
  });
});
