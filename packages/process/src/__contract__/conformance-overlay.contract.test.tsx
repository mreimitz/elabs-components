...
/**
 * Contract probe for ConformanceOverlay (`packages/process/src/conformance-overlay/conformance-overlay.tsx`), derived from its
 * `Default` story (packages/process/src/conformance-overlay/conformance-overlay.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../conformance-overlay/conformance-overlay.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `process-conformanceoverlay--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("ConformanceOverlay contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="conformance-overlay" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="conformance-overlay"]')).not.toBeNull();
  });
});
