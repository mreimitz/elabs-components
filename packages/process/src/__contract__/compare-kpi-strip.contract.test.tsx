...
/**
 * Contract probe for CompareKpiStrip (`packages/process/src/process-compare/compare-kpi-strip.tsx`), derived from its
 * `Default` story (packages/process/src/process-compare/compare-kpi-strip.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../process-compare/compare-kpi-strip.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `process-processcompare-comparekpistrip--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("CompareKpiStrip contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="compare-kpi-strip" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="compare-kpi-strip"]')).not.toBeNull();
  });
});
