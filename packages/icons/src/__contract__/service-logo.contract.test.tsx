...
/**
 * Contract probe for ServiceLogo (`packages/icons/src/service-logo.tsx`), derived from its
 * `Default` story (packages/icons/src/service-logo.stories.tsx). See scripts/gen-contract-tests.mjs.
 */
import { describe, it, expect, afterEach } from "vitest";
import { render, cleanup } from "@testing-library/react";
import { createRef } from "react";
import * as stories from "../service-logo.stories";
...
function contractIt(assertion: string, name: string, fn: () => void) {
  const key = `icons-servicelogo--default|jsdom|jsdom|${assertion}`;
  const reason = KNOWN_FAILURES[key];
  if (reason) return it.fails(`${name} (known failure: ${reason})`, fn);
  return it(name, fn);
}

describe("ServiceLogo contract", () => {
  ...
  contractIt("data-slot", 'exposes data-slot="service-logo" on its root', () => {
    const { container } = render(<Component {...args} />);
    expect(container.querySelector('[data-slot="service-logo"]')).not.toBeNull();
  });
});
