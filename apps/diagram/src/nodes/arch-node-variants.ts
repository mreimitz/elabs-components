 ...
 *   outline so the dash shows at all). In the `card` look `datastore` rounds the card's
 *   bottom (a cylinder cue) and `queue` gets a left rail. In the `icon` look there is no
 *   frame to shape, so the cue goes on the MARK: `actor` a circle, `datastore` a box with a
 *   rounded bottom, `queue` a pill — each drawn in the mark's own colour (`border-current`,
 *   so it follows the tone the way the glyph does) and reached through the mark's
 *   `data-slot`, the way `flowToneVariants` reaches `data-flow-tone-part`s, so one config
 *   styles the node. A vendored mark is an image, so without this a service, a datastore
 *   and a queue with vendor icons looked identical (wave-1 review m1).
 *
...
export const archNodeVariants = cva("flex flex-col", {
  variants: {
    ...
    kind: {
      ...
      // Shape cues that differ per look: see `compoundVariants`.
      datastore: "",
      queue: "",
  ...
  compoundVariants: [
    {
      ...
      kind: "datastore",
      className:
        "**:data-[slot=arch-mark]:rounded-t-sm **:data-[slot=arch-mark]:rounded-b-xl **:data-[slot=arch-mark]:border-2 **:data-[slot=arch-mark]:border-current **:data-[slot=arch-mark]:p-1",
    },
    { variant: "card", kind: "datastore", className: "rounded-b-xl" },
    {
      ...
      kind: "queue",
      className:
        "**:data-[slot=arch-mark]:rounded-full **:data-[slot=arch-mark]:border-2 **:data-[slot=arch-mark]:border-current **:data-[slot=arch-mark]:px-4 **:data-[slot=arch-mark]:py-1",
    },
