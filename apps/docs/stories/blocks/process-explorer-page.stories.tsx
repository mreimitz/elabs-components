...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      ...
      description: {
        component:
          'A map-first process-mining workspace. The six KPIs are one ribbon and every control lives in a toolbar, so the height of the screen belongs to the process map and nothing floats over it. Beside the map: statistical insights (throughput distribution against the SLA, the slowest hand-overs with a mean-line `analytics` overlay, the weekly trend with a linear-trend analytic on the median, box and violin plots by region and channel), the variants as colour strips, the selected element with the distribution behind its median, and deviations from the reference model. A dock holds the dotted chart, the performance spectrum, a workload `HeatmapChart` and a resource `BarChart` — both `scrollbar="auto"`, the bar chart with its own mean line — and the case table; Replay animates every order as a token. One `useProcessExplorer` instance drives every view, so the last interaction wins everywhere.\n\nCopy-own it: `npx shadcn add process-explorer-page`.',
      },
    },
  },
  tags: ["autodocs"],
