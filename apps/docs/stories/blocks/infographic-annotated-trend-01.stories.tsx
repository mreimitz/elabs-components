...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      ...
      description: {
        component:
          "Answers “what happened, and when?” — a weekly line with up to three events written as declarative `annotations`, each connected to the data point it explains, never a legend. A linear `analytics` trend and a 4-week `window` rolling average draw alongside it, named in the method note. `ChartFrame` carries the editorial chrome: the finding as the title, the method note, a byline and the source row. Under 480 px the notes become numbered markers with a key under the plot. Semantic tokens only; reads in both themes.\n\nCopy-own it: `npx shadcn add infographic-annotated-trend-01` (pulls `kpi-card-parts`).",
      },
    },
  },
  tags: ["autodocs"],
