...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      ...
      description: {
        component:
          'Two selection FIELDS on one `createLocalSelectionDriver()`. An `AreaChart` of incidents per day with `selectionGestures={["range"]}` on its time axis writes the `day` field (every day in the range, as dates); a horizontal `BarChart` of incidents by service writes the `service` field by click, Ctrl/Cmd+click or an axis range. Each chart paints its OWN field through `useSelectionDriver(driver, { field })`, while the `DataTable` of the pager log and the four `MetricCard`s show the INTERSECTION — a row passes a field when the field carries no selection or holds its value. Severity is a `StatusBadge` mapped once (`SEV1` → destructive + glyph), so colour is never the only channel.\n\nCopy-own it: `npx shadcn add incident-explorer-01`.',
      },
    },
  },
  tags: ["autodocs"],
