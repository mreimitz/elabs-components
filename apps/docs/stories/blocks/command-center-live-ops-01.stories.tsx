...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      ...
      description: {
        component:
          'An operations wall that moves: a `LiveLineChart` streaming one reading a second and easing to each new value, three `Gauge` dials with their targets drawn on the dial, a `Timeline` of what the on-call already knows, services sorted worst first with a `Sparkline` and a status badge each, and a `HeatmapChart` of when the load arrives with `scrollbar="auto"` and a responsive `maxVisibleItems={{ base: 12, medium: "auto" }}` so the hours stay readable on a narrow tile. The headline is computed from the services, so it is never out of step with the list. `paused` freezes the stream.\n\nCopy-own it: `npx shadcn add command-center-live-ops-01`.',
      },
    },
  },
  tags: ["autodocs"],
