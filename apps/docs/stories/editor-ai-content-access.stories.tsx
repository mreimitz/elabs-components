...
} from "@elabs-ai/components-editor/markdown";
...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: `
Engine-agnostic \`EditorContentAccess\` interface for AI-driven editing in \`@elabs-ai/components-editor\`.
...
- **\`monacoContentAccess(editor)\`** — wrap a \`MonacoCodeEditor\` ref (also returned by \`CodeWorkspaceHandle.getActiveEditor()\`). Exported from \`@elabs-ai/components-editor\`.
- **\`proseMirrorContentAccess(deps)\`** — wrap a Milkdown view (markdown-fidelity by default: selection serialized to markdown; incoming text parsed as markdown fragment). Exported from \`@elabs-ai/components-editor/markdown\`.
