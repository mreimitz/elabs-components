import type { Meta, StoryObj } from "@storybook/react-vite";
import { ThemeProvider } from "@elabs-ai/components-tokens";
import CaseStudiesPage from "@/components/case-studies-page/case-studies-page";

/**
 * Renders the SHIPPED registry page, not a copy of it — see `.claude/rules/registry.md`.
 */
const meta = {
  component: CaseStudiesPage,
  title: "Patterns/Templates/Pages/Case Studies",
  parameters: {
    layout: "fullscreen",
    docs: {
      subtitle: "For a customers / case-studies index page",
      description: {
        component:
          "The customers page: the wall of case studies filterable by industry, the big-number outcomes, a logo strip and the closing CTA.\n\nCopy-own it: `npx shadcn add case-studies-page`.",
      },
    },
  },
  tags: ["autodocs"],
  // The navbar's <ThemeSwitcher /> reads the @elabs-ai/components-tokens React
  // context, so the page needs a real provider — the global preview decorator
  // only writes the `data-theme` attribute. In a consuming app this sits at the
  // root. It mounts DEEPER than the preview's own theme boundary, so a
  // `STORYBOOK_THEME=<slug>` sweep still wins (child effects flush first).
  decorators: [
    (Story) => (
      <ThemeProvider>
        <Story />
      </ThemeProvider>
    ),
  ],
} satisfies Meta<typeof CaseStudiesPage>;

export default meta;
type Story = StoryObj<typeof meta>;

export const Default: Story = {};
