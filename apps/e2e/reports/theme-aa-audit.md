...
## Addendum (hand-recorded, not generated) — chart accent palette, #179

**Everything above this line is generated by `pnpm audit-artifact`; everything below is
hand-recorded.** The generator walks the twelve-series categorical ramp (`--chart-1..12`)
only — it does not yet walk the ordinal ramps RM-018 added (`--chart-seq-*`,
`--chart-mono-*`, `--chart-div-*`, `--chart-accent`), so this pairing cannot appear in the
tables above the way the categorical rows do. Teaching the generator about the ordinal
ramps is a follow-up, not done as part of #179; until then, `pnpm audit-artifact:check`
judges only the generated portion of this file and does not see this section — and, having
anything appended below the generated content at all, will now report this file as stale
until it is taught to expect this addendum too. That is accepted, not an oversight: see the
report accompanying #179 for why nothing in this repo's write set can fix that gate.

| token | vs | ratio | min | |
| --- | --- | --- | --- | --- |
| `--chart-accent` | `--chart-background` (`light`) | 1.42 | 3.0 | ⚠️ |
| `--chart-accent` | `--card` (`light`) | 1.42 | 3.0 | ⚠️ |
| `--chart-accent` | `--chart-background` (`dark`) | 11.25 | 3.0 | ✅ |
| `--chart-accent` | `--card` (`dark`) | 11.25 | 3.0 | ✅ |

Marked `⚠️`, not `❌`: `--chart-accent` is declared `var(--chart-1)`
(`packages/tokens/src/themes/light.css`, `themes/dark.css`), so on `light` it is already
covered by the `light` theme's chart-ramp exemption above (`--chart-1`'s own row, also
`⚠️`) — this is the same measured pairing reached through a second token name, not a new
below-bar pairing.

What is new, and why it earns its own record rather than riding silently on `--chart-1`'s
row: the accent palette's whole design premise is "one hero colour, the loudest mark on
the plot" (`ChartPalette: "accent"`, `packages/charts/src/charts/chart-context.tsx`), drawn
over the neutral `--chart-mono-*` ladder. Measured against that premise on `light`, the
hero is quieter than its own usual context colours — `--chart-mono-2`/`-4`/`-6`, the three
steps `chart-ramps.stories.tsx` pairs it with — which measure 3.10 / 5.70 / 10.21:1, and
quieter than the sequential ramp's own declared *quiet* step `--chart-seq-1` (1.62:1). So
on `light` the hero reads as the faintest ink on the plot, not the loudest.

Decided 2026-09-04: accept and document (issue #179), extending the existing light-theme
chart exemption rather than opening a second one. The systemic repair under consideration
is the same one the ramp exemption already names — darken `--chart-background` in the
light theme — plus a second route weighed and declined here, deriving a per-theme accent
(lime on dark, a darker lime-family colour on light). Both were declined as too wide a
reach for this track, not as wrong ideas. Full record: `.claude/rules/theming.md` (the
chart-accent bullet, sibling to the chart-ramp exemption) and
`packages/tokens/src/charts-contrast.test.ts` (the comment on `CHART_1411_EXEMPT`). No
token value changed to produce this entry — both reference themes render byte-identically
to before #179.
