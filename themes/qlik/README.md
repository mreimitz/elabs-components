...
| Primary / success         | `#00873D`, hover `#006C31`, pressed `#004C22`, white ink        | `--primary`, `--primary-text`, `--success`             |
...
| Sidebar                   | white, 1 px 8 % edge, active row 5 % wash, 4 px radius          | `--sidebar*`                                           |
| Elevation                 | `0 1px 2px rgba(0,0,0,.15)` + 1 px 5 % hairline ring, no border | `--shadow-strength: 1.5`, `--shadow-ring-color`        |
| Radius                    | 4 px controls and cards (inputs 3 px in MUI parts)              | `--radius-base: 0.25rem`                               |
...
- `--font-sans`: **Source Sans 3** — the open-source successor of Source Sans Pro, which is
  ...
  that import.
- `--font-display`: **Inter**, the brand guideline's primary typeface (Qlik Cloud's
  "expressive" theme also sets headings in Inter). Weights: headlines Bold, subheads
  Semibold, body Regular — the engine's type roles already use 600/400.
