...
Everywhere else, `@elabs/components-*` resolves to TypeScript **source** via the `exports`
...
- `@elabs/components-tokens` fonts copied to `dist/fonts/fonts/…`, so every `@font-face` 404'd
- `@elabs/components-maps` / `@elabs/components-editor` CSS extracted by esbuild and then orphaned, so
  the components shipped unstyled
- `@elabs/components-editor`'s `./monaco-environment` subpath pointing at raw `.ts`
...
workspace, rewrites each `@elabs/components-*` placeholder version to a freshly packed
