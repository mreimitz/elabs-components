...
- the associative BI suite's model: a selection is provisional (green/grey preview) until ✓, click-outside or Enter; ✕/Esc cancels; click toggles while open; lasso/range are toolbar toggles on the visualization. the analytics-pane BI suite and the report-builder BI suite apply immediately and offer Keep-only/Exclude from the tooltip. Our theme-fidelity rule: a the associative BI suite theme may flip defaults, never inject product code.
...
- Stories: `explicit` session on a bar chart (click, click, range, ✓ → one intent with the union; ✕ → none and paint restored), `immediate` on a scatter (each lasso → one intent), the linked-charts story (assert the bar chart's `data-selection` states after a scatter lasso via the driver), toolbar keyboard run, a the associative BI suite-theme story showing the provisional paint styled by tokens only.
...
Chart tests, `chart-frame.test.tsx`, contract tests, `pnpm check`, Storybook Chromium light + dark, the default theme and the associative-suite brand theme, three widths, keyboard GIF.

## Orchestrator notes

The intent shape is the contract the dashboard revival (`parked/dashboard-pack/REVIVE.md`) will consume — do not add fields the parked driver cannot map; add to `gesture` instead. Ask the maintainer whether `explicit` should be the default when the associative-suite brand theme is active (frame-level default, allowed) — recommended yes.
