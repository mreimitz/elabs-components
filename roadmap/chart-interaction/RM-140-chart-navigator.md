...
- Qlik shows a mini chart automatically "when the number of dimension values exceeds the width", with `scrollbar: "miniChart" | "bar" | "none"`, `scrollStartPos` 0/1 and, for large data, "a condensed version … in gray … the very low and the very high values are still visible". Highcharts' navigator borrows series 0 grouped at 2 px; Recharts re-renders the child chart small (costly); MUI X and Highcharts (v11.2) expose the handles as sliders with keyboard.
...
- `navigator-window.ts` — `NavigatorWindow` (RM-136) with `clampWindow`, `minSpan` (default: 5× the median step, Highcharts' rule; index: 3), `align` (`"end"` starts at the latest data, Qlik `scrollStartPos: 1`), `shiftWindow`, `zoomWindow`, and `indexToTime` / `timeToIndex` so the same strip serves both models.
- Gestures (`use-navigator-gestures.ts`, pointer events, `setPointerCapture`): drag the window, drag a handle, click outside the window to centre it there (ECharts "click to locate"), wheel over the strip pans, two-finger horizontal pan on touch, pinch zooms; `realtime` updates the plot on every frame (rAF-throttled) and commits on release; `onWindowChange(window, { phase: "move" | "commit" })`.
- Time-series shell: `scrollbar` (default `"miniChart"` when `rows > maxVisiblePoints` (default 2 000, Qlik's cap) or when `window` is controlled; `"bar"` = a plain ui `ScrollBar` strip; `"none"`), controlled `window` / `defaultWindow` / `onWindowChange`. The shell keeps feeding `xDomain` exactly as today; nothing in the plot changes.
