...
- Qlik: "You draw your selections on the y-axis or the x-axis. For an axis with measure values, you can click on the range bubble to enter a specific numeric value"; supported on bar, box, combo, distribution, histogram, line, scatter; the gesture starts "just outside the chart area" on the axis; on a time axis it "select[s] all data values (even those that are not visible)". Vega-Lite gets the same with `encodings: ["x"]`; ECharts with `lineX`/`lineY`.
- No library besides Qlik ships the editable bubble; none documents keyboard for it — the APG multi-thumb slider is the fit.
...
The measure-axis rule is the one people get wrong: it selects **dimension values**, never "rows with value between". Quote the Qlik page in the PR and test it with a series where two categories share a value.
