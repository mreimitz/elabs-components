 ...
 * the durable CI gate (@elabs-ai/components-eslint-config `brand/no-raw-*`); the regexes mirror
...
// Mirror of @elabs-ai/components-eslint-config rules/brand-tokens.js + scripts/check-raw-palette.mjs.
...
if (colors.length)
  ctx +=
    ...
    fmt(colors);
ctx += `\n\nSee the app's CLAUDE.md (Non-negotiable rules) — enforced by @elabs-ai/components-eslint-config (brand/no-raw-font-size, brand/no-raw-color).`;
