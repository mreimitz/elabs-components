...
`@elabs-ai/components-*` in the client. Three of the four problems the
...
>
> `ChartFrame` (`@elabs-ai/components-charts`) already puts the toolbar
...
> system ships for it.
>
> Use `ExpandDialog` (`@elabs-ai/components-ui`): enlarged content on
...
>
> `ToolResultCard` (`@elabs-ai/components-ai`) has an `actions` slot on
