...
> **Withdrawn in 5.3.0 — the dashboard sheet subpath.**
> `@elabs-ai/components-charts/dashboard` and `@elabs-ai/components-charts/dashboard/test`
> (ADR 0037) shipped in 5.0.0–5.2.0 and are **not published from 5.3.0 on**. The surface was
> parked on 2026-09-22 while its authoring experience is reworked; the decision behind it is not
> reversed. If you import either subpath, pin `@elabs-ai/components-charts@5.2.0` until it
> returns. The static KPI dashboard — `MetricGrid`/`MetricCard`/`ChartFrame` off the main
> `@elabs-ai/components-charts` barrel — is unaffected, and its playbook still ships.
