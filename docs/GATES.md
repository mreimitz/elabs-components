...
- The runtime import closure of `packages/charts/src/definitions/**` and `charts/props/**` reaches only the pure `@elabs-ai/components-ui/definition` base and allow-listed leaves, walked transitively — never React, visx, d3, motion or `@elabs-ai/components-ui` at runtime (`import type` is exempt; `*.test-d.ts` is exempt) (ADR 0042 §11, RM-172). (`charts-definitions-pure`)
