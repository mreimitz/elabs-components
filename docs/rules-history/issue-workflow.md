...
Moved out of .claude/rules/issue-workflow.md on 2026-09-06 so it stops loading into every session. This is the record of incidents, measurements and rejected alternatives behind the rule; the binding rule itself lives in the rule file.

Every section below is the pre-condensation text, verbatim. The "Superseded rules" section was replaced on 2026-09-06 by the fix-in-scope rule (fix a small, in-scope finding in the PR that found it; file an issue only for what is left behind; route through the analyst only for P0/P1 or an unknown cause, batched). Everything else was condensed into the rule file; the full original wording is kept here.

## Superseded rules (replaced 2026-09-06)
...
## Machine-posted comments carry their provenance (#78) — origin
...
### Marker rule — original full text
...
### Parser design, fix rounds 7–12 and the open bypass axes (#96, #126)
...
- **A fifth bypass axis, undeclared until now: ASSIGNMENT SYNTAX the scan's
  ...
  (examples, not exhaustive — see #126): append (`CMD+="…"`), indexed element
  (`CMD[0]="…"`), array literal (`A=("…")`), associative array
  (`declare -A M; M[k]="…"`), ANSI-C quoting (`CMD=$'gh …'`, confirmed on
  bash and zsh), and `printf -v` (`printf -v CMD '%s' "gh …"`, confirmed on
  bash and zsh). PRE-EXISTING on merge-base `main` — round 12 neither opened
  ...
  class; new forms may surface. Needs a matching dereference on the same line
  to actually execute — the payload alone in an unread variable does nothing.
  Not yet closed by any round.

### Gate semantics — original full text
