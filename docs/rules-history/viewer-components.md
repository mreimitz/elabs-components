# viewer-components — history

Moved out of .claude/rules/viewer-components.md on 2026-09-06 so it stops loading into every session. This is the record of incidents, measurements and rejected alternatives behind the rule; the binding rule itself lives in the rule file.

## Original rule text before condensation (verbatim, 2026-09-06)

The full pre-condensation body follows (the path-scoped YAML frontmatter is omitted — it is unchanged in the rule file). Every binding instruction in it was carried into the condensed rule; the justifications behind each bullet, the incident narratives (the `image` manifest that claimed `zoom`/`rotate` for a renderer that implemented neither, the missing-peer that surfaced as `parse-failed`, SheetJS opening a damaged workbook as one junk cell, nested scrollers reading as a failed render), the worked examples and the reasoning behind each rejected alternative (sanitize-and-inject, a `never` fallthrough, the CSS Custom Highlight API, porting `_code-block-theme.ts`, a per-theme fallback chain) are what was dropped and live only here. Relative links below were re-pointed so they resolve from `docs/rules-history/`.

# File viewer components (@elabs-ai/components-viewer)

`@elabs-ai/components-viewer` renders **a file the app did not write** —
uploaded, fetched from a signed URL, or produced by an agent — themed and accessible.
Durable record: [`docs/ADR/0024-viewer-package.md`](../ADR/0024-viewer-package.md).
`@elabs-ai/components-editor` is its sibling: `editor` is for source you
...
Durable record: [`docs/ADR/0026`](../ADR/0026-viewer-view-state-and-chrome-parts.md).
...
`@elabs-ai/components-ui` components. It never returns an HTML string, never
...
- **`markdown`** claims `.md` and `text/markdown` and renders a **document**
  (Streamdown → the same `Prose*` primitives `@elabs-ai/components-ai` uses).
...
`@elabs-ai/components-ai`'s `_code-block-theme.ts` (oklch→hex + a
...
reader's flat heading list. Same seam as`@elabs-ai/components-ai`'s
`MarkdownView`.

## Pointing at part of a document

A citation ("this answer came from that paragraph") and find-in-document are the
same layer. Durable record: [`docs/ADR/0025`](../ADR/0025-document-highlighting-and-addressing.md).
...
- **One address union, three kinds** — `quote` (a snippet — the interoperable one,
  ...
  `@elabs-ai/components-ui` (`src/lib/document-address.ts`) because
...
`fileIconFor`, `FileCategory` — lives in `@elabs-ai/components-ui`
