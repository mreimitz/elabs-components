# design-first — history

Moved out of .claude/rules/design-first.md on 2026-09-06 so it stops loading into every session. This is the record of incidents, measurements and rejected alternatives behind the rule; the binding rule itself lives in the rule file.

## Origin (the incident behind the rule)

The rule's original title was `# Design first (end-user surfaces start as design, not as assembly)`, and the five steps sat under the heading `## Before scaffolding ANY net-new screen / page / major surface`.
...
## Examples and justifications dropped from the five steps
...
## Patterns over instances (examples dropped)
...
## Delegating UI work to agents (justification dropped)
...
## Verify like a designer, not only like a compiler (justification dropped)
