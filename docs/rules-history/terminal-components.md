# terminal-components — history

Moved out of .claude/rules/terminal-components.md on 2026-09-06 so it stops loading into every session. This is the record of incidents, measurements and rejected alternatives behind the rule; the binding rule itself lives in the rule file.

## Original rule text before condensation (verbatim, 2026-09-06)

The full pre-condensation body follows (the path-scoped YAML frontmatter is omitted — it is unchanged in the rule file). Every binding instruction in it was carried into the condensed rule; what was dropped and lives only here is the justification prose around each rule (why console-ness is a surface rather than twelve components, why the context must stay minimal, why `ch` alignment breaks at a font fallback), the ADR 0033 incident narrative (twelve correct components and a wrong assembled screen, the status bar hanging like a page footer), the fidelity-axis comparison table and the box-drawing screen-reader justification, the bottom-anchoring measurement (`justify-end` leaving 190px unreachable on `patterns-templates-terminal-agent-session--default` at 1440×900 and why no gate could see it), the drift-repair story behind the promotion rule, the measured contrast failures behind the three-grounds colour rule (2.57:1, 1.2:1 and 3.61:1 from real axe runs, the `--terminal-selection` token comment's enumerated inks), the elevation and type-role restatements of the styling rule, the cross-cutting restatements in the builder checklist, and the upstream-provenance rationale. The original cited other rules by bare file name, so no relative links needed re-pointing.
