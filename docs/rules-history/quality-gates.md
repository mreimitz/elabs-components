...
Moved out of .claude/rules/quality-gates.md on 2026-09-06 so it stops loading into every session. This is the record of incidents, measurements and rejected alternatives behind the rule; the binding rule itself lives in the rule file.
...
[`docs/GATES.md`](../GATES.md).** Read it when you add or change a gate, when one
...
- This is a **nudge that needs judgment** about "larger", so it is intentionally not a
  ...
  - **Verified against real bytes, not only fixtures.** Replayed over the transcripts in
    this repo's Claude Code transcript directory (`~/.claude/projects/<project>/`), it now nudges
