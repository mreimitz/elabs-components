# brand-ui-root-cause-analyst agent — history (moved on 2026-09-06)

Moved out of `.claude/agents/brand-ui-root-cause-analyst.md` on 2026-09-06 when the agent was cut to a batch contract with a capped spec per finding. The live file keeps its frontmatter (`name`, `description`, `tools`, `model` — now `sonnet`, with the sequential-thinking tool removed); the body below is the previous prose, verbatim.

## Original agent body (verbatim, 2026-09-06)
