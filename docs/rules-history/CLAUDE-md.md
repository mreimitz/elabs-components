...
Moved out of CLAUDE.md on 2026-09-06 so it stops loading into every session. This is the hand-written text the lean CLAUDE.md dropped or condensed, kept verbatim; the binding project memory is CLAUDE.md itself, and the generated decisions table that sat between the two halves below still lives there.

## Original hand-written text (before the shrink)
...
_(the generated `brand-ui:gen:decisions` region sat here — it is still in CLAUDE.md, regenerated by `pnpm gen`)_
