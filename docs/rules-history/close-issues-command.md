# close-issues command — history (moved on 2026-09-06)

The full text of `.claude/commands/close-issues.md` as it stood before the 2026-09-06 rewrite, kept verbatim for the measurements and incident notes it records.
...
allowed-tools: Agent, Task, TaskOutput, TaskStop, SendMessage, Monitor, Workflow, Skill, Read, Write, Edit, Grep, Glob, TodoWrite, Bash(node:_), Bash(pnpm:_), Bash(gh:_), Bash(git:_), Bash(ls:_), Bash(rg:_), Bash(find:_), Bash(mkdir:_), Bash(chmod:_), Bash(jq:_), Bash(bash:_), Bash(python3:_), Bash(grep:_), Bash(awk:_), Bash(sed:_), Bash(wc:_), Bash(diff:_), Bash(cp:_), Bash(rm:_), Bash(sleep:_), Bash(until:\*)
