# interaction-guidelines — history

Moved out of .claude/rules/interaction-guidelines.md on 2026-09-06 so it stops loading into every session. This is the record of incidents, measurements and rejected alternatives behind the rule; the binding rule itself lives in the rule file.

## Original rule text before condensation (verbatim, 2026-09-06)

The full pre-condensation text follows. Every binding instruction in it was carried into the condensed rule; what was dropped and lives only here is the library-vs-app rationale, the focus-fixup explanation behind the `aria-disabled` submit exception, the microtypography gate's issue reference, script path, scanned-attribute list, self-test and CI wiring, the Tailwind v4 Preflight story behind the global pointer cursor, the worked examples and glosses (`email`/`tel`/`url`/`number`, `onPaste`+`preventDefault`, `10&nbsp;MB`, `⌘&nbsp;K`, MetricCard/DataTable numeric cells, "kills the 300ms tap delay", "prevents CLS", "the silent culprit", "so native scrollbars/inputs match"), the ESLint plugin naming for the WP-10 gate, and the closing source paragraph. It contains no relative links, so nothing needed re-pointing.
