# chart-components — history

Moved out of .claude/rules/chart-components.md on 2026-09-06 so it stops loading into every session. This is the record of incidents, measurements and rejected alternatives behind the rule; the binding rule itself lives in the rule file.

What follows is the complete text of the rule as it stood before the condensation (frontmatter excluded — it stays on the rule), kept whole so every dropped sentence stays in its original context. Where this text and the rule file disagree, the rule file wins.
