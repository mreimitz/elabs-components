# editor-components — history

Moved out of .claude/rules/editor-components.md on 2026-09-06 so it stops loading into every session. This is the record of incidents, measurements and rejected alternatives behind the rule; the binding rule itself lives in the rule file.

## Original rule text before condensation (verbatim, 2026-09-06)

The full pre-condensation body follows (the path-scoped YAML frontmatter is omitted — it is unchanged in the rule file). Every binding instruction in it was carried into the condensed rule; what was dropped and lives only here is the explanatory prose around each bullet: the wrap-an-engine framing beside `@elabs-ai/components-flow` / `@elabs-ai/components-data`, why the theme bridge rasterizes on a 1×1 canvas as a fallback (`getComputedStyle` won't serialize `oklch()`), the reasoning behind owning the footnote render (Streamdown's default hast has broken in-page anchors + `target=_blank`), the KaTeX CSS cost rationale, the `resolveUrl`/`resolveWikilink` precedent for the academic props, why `IterationCell` was extracted (a `forwardRef` self-reference), the #223 WYSIWYG iterate/pivot account, and the #220/#221 calc-authoring design record (the `slashMenu`/`evaluate` pattern it mirrors, the AA-verification note on the `--calc-*` tokens). The original contains no relative markdown links; its `@.claude/rules/…` references are kept as written.
