# ai-chat-components — history

Moved out of .claude/rules/ai-chat-components.md on 2026-09-06 so it stops loading into every session. This is the record of incidents, measurements and rejected alternatives behind the rule; the binding rule itself lives in the rule file.

What follows is the complete body of the rule as it stood before the condensation (the `paths:` frontmatter, which stayed byte-identical in the rule file, is omitted), kept whole so every dropped sentence stays in its original context. Every binding instruction in it was carried into the condensed rule; the incident narratives, measurements, review-thread reasoning and "why this gate exists" stories are what was dropped and live only here. Where this text and the rule file disagree, the rule file wins.
...
- **`ContextPanel` vs `ContextRail` — pick by what the panel must DO, not by
  package proximity (ADR [0035](../ADR/0035-context-rail-and-side-dock.md)
...
- **Not to be confused with `@elabs-ai/components-flow`.** `Canvas`/`Node`/`Edge`/
  ...
  instead — see [ADR 0018](../ADR/0018-dual-react-flow-canvas-surfaces.md) and
