# styling-and-tokens — history

Moved out of .claude/rules/styling-and-tokens.md on 2026-09-06 so it stops loading into every session. This is the record of incidents, measurements and rejected alternatives behind the rule; the binding rule itself lives in the rule file.

Every section below reproduces the original text of the rule **verbatim**, in its original
order, so the sentences that were cut keep their context; the instruction inside each bullet
still lives in `.claude/rules/styling-and-tokens.md`, which is the only copy that binds. Two
declared transformations only: the original's section headings are demoted one level to nest
under this file's own, and its two rule-relative `@.claude/rules/…` imports are rendered as
backticked repo-root paths, since the import form resolves only inside the rules directory.

## Base bullets — original text

Kept in the rule: every instruction. Dropped: the longer example-utility list, the
boundary-hook note (the hook prints its own message), and the surrounding phrasing.
...
## Focus — original bullet

Kept in the rule: the four `focus-ring*` utilities, the banned one-layer shape, the
`[--focus-ring-color:…]` retarget and the ADR 0027 pointer. Dropped: the two-layer
(`--ring-contour`) explanation, the measured contrast of the aliased ring on `light`, the #67
sweep reference and the pointer to the theming rule (always-on in its own right).
...
## `border` vs `border-strong` — original bullet

Kept in the rule: both rungs, `border-input` as the subtle rung, the decision test and the
same-tone caveat. Dropped: the issue reference, the 1.4.11 exemption reasoning, the named
form-control list and the dated ADR 0010 Amendment narrative for `--input`.
...
## Which status rung a graphical MARK reaches for (#381) — original bullet

Kept in the rule: the three rungs, the surfaces the fill rung is guaranteed against, the
locking test and the decision test. Dropped: the `--warning` amber history, the
`@elabs-ai/components-editor` entity-chip regression, and the labelled-plate paragraph (a
filled `Badge`/`StatusBadge` plate is 1.4.11-exempt but shares the token, so the ≥3:1 bar
...
- **Which status rung a graphical MARK reaches for (#381).** A status tone ships three
  ...
  - **`text-<tone>-foreground` is ONLY the ink on a solid `bg-<tone>` plate.** It is
    ...
    #381 had to repair in `@elabs-ai/components-editor`'s entity chip.
...
## Cascade, new tokens, `@source`, `MetricCard` — original bullets

Kept in the rule: every instruction, shortened.
...
- **KPI tile ownership:** the canonical KPI tile (`MetricCard`) is owned by `@elabs-ai/components-ui`; `@elabs-ai/components-charts` and `@elabs-ai/components-editor` re-export it (ADR 0012).

## Typography scale (#187) — original role bullets

Kept in the rule: the role names, the four bundled dimensions, `kpi`/`tabular-nums`,
`code`/`font-mono`, the font seams. Dropped: the `@utility` justification, the
`text-body leading-loose` example and the "once they land (#188)" qualifier — the
`<Heading>`/`<Text>` primitives have since landed in `@elabs-ai/components-ui`.
...
## Type IS density-aware (#340) — original bullet, with the per-screen measurements

Kept in the rule: the factor triple, the legibility floor, comfortable = unchanged, the
role-typed-text-only reach, the `text-xs` → `text-meta` weight/tracking shift, the mechanism
and the `pnpm text-scale:check` ratchet. Dropped: the reversal date, the half-rate rationale,
every measured count (272 raw uses / 96 files, the per-package split, the four per-screen
figures and their confirmation status, the registry-tree tally), the custom-property-cycle
explanation, and the pointers to the locking test
(`packages/tokens/src/density-type-scale.test.ts`) and the `Foundations/Typography → Density
scale` story.
...
## "Prose" vs "label-like" description text (#339) — original section

Kept in the rule: default full-width, the opt-in `measure` prop, unconditional
`text-balance`, the decision test and "no third variant". Dropped: the framing paragraph,
the named `CardDescription`/`AlertDescription` examples and the reading-speed justification.
...
## Surface separation (#187) — original section

Kept in the rule: the four gestures and their tokens, no redundant border, no generic
`<Surface>`, the chrome < canvas < raised invariant with its `L(--background) −
L(--sidebar) ≥ 0.02` bound, and the two gates. Dropped: the #391 low-chroma discussion
beyond one sentence (including its ΔE ≈0.012 measurement and the decoration-dial
disclaimer), the past `--sidebar == --background` regression, and the "narrow + honest" gate
framing (what `pnpm separation:check` flags and what it leaves to the visual reviewer).
...
## Elevation (ADR 0020) — original section

Kept in the rule: never hand-roll, the seven rungs, `shadow-ring-*` for floating surfaces
with no border, resting surfaces keep their border, plain rungs for chips,
`shadow-hairline`, reach by role, the theme-level retune knobs (ink, strength, hairline), ring ≠ 1.4.11 cue, and
`pnpm elevation:check`. Dropped: the layer/alpha construction of the stack and its
`flornkm/shadow-plugin` provenance, the baked-in-hairline / "double edge" explanation, the
`lg` offset/blur figures, the 5%/18% hairline alphas, the sidebar `outline` menu-button
example, the shadowless-decoration cascade note and the gate's assertion list.
