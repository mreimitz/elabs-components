...
Moved out of .claude/rules/decoration.md on 2026-09-06 so it stops loading into every session. This is the record of incidents, measurements and rejected alternatives behind the rule; the binding rule itself lives in the rule file.

What follows is the complete text of the rule as it stood before the condensation, kept whole so every dropped sentence stays in its original context. Where this text and the rule file disagree, the rule file wins.

## Notes from the condensation

- The example scenario story was cited as `scenarios-agentic-ai-workspace--default`; the story's title is `Patterns/Scenarios/Agentic AI Workspace`, so the condensed rule cites `patterns-scenarios-agentic-ai-workspace--default`.
- The condensed rule adds two pointers the original lacked: `pnpm decoration-collapse:check` (the gate that guards rule 2, "never inside a control") and `docs/ADR/0020-stacked-elevation-ramp.md` (the shadowless mechanism).
- The original's closing pointer to the `styling-and-tokens` and `editor-components` sibling rules was dropped: the first is always-on and the second loads on its own path scope, so the `@`-reference only pulled `editor-components` into every tokens session for nothing.
- The bare "#29 item 3" citation stays in the rule file; `docs/GATES.md` and the issue-citations gate both describe it as deliberately unregistered there.

## The rule as it stood before condensation (verbatim)
