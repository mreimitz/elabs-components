...
`@qlik-coe-emea/qlabs-components-ui` ships `LocaleProvider` / `useLocale()` with `locale`, `dir`, `t`,
...
the tree** (verified absent); `@qlik-coe-emea/qlabs-components-ui` already depends on ~25 `@radix-ui/*` packages
at the `^1.x`/`^2.x` line.

Separately, `@qlik-coe-emea/qlabs-components-ai` hardcodes locale in formatters: `context.tsx` has 11
`new Intl.NumberFormat("en-US", …)` and `commit.tsx` a `new Intl.RelativeTimeFormat("en", …)`.
`@qlik-coe-emea/qlabs-components-ai` depends on `@qlik-coe-emea/qlabs-components-ui` (`workspace:*`), so it **can** consume `useLocale()`.
...
### (b) Microcopy / format-locale seam in `@qlik-coe-emea/qlabs-components-ai`

- The 11 `Intl.NumberFormat` and the `RelativeTimeFormat` in `@qlik-coe-emea/qlabs-components-ai` read `locale`
  from `useLocale()` instead of the hardcoded `"en-US"`/`"en"`. `@qlik-coe-emea/qlabs-components-ai` already
  depends on `@qlik-coe-emea/qlabs-components-ui`, so this is an in-graph consumption, not a new edge.
...
- **Non-hook formatting path:** some `@qlik-coe-emea/qlabs-components-ai` helpers may format outside a component
...
- `+` `@qlik-coe-emea/qlabs-components-ai` numbers/dates/relative-times honor the active locale; the en-US leak is
  closed; components stay presentational.
- `+` English-only consumers see zero behavior change (opt-in seam).
- `−` One new MIT runtime dep in `@qlik-coe-emea/qlabs-components-ui` and a lockfile update (human-approved).
