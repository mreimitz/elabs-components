...
No `@elabs-ai/components-*` package shipped a mention-capable input.
...
Ship `MentionInput` in **`@elabs-ai/components-ui`** as a compound
component over a real `<textarea>`, with its own listbox.

### 1. `@elabs-ai/components-ui`, not `-ai`, not the registry
...
**`@elabs-ai/components-ai` gains no export and no source change.**
