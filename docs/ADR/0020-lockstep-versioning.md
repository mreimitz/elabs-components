...
| **Independent per-package SemVer**                 | Consumers install several `@elabs-ai/components-*` packages that share tokens, peers and a theme contract. Independent versions make "which set is compatible" a consumer problem and let the plugin drift from the library it documents — the exact failure #104 was filed to end.                                                                                                                                                                                        |
