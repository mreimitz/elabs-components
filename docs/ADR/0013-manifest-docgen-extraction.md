...
`@elabs-ai/components-cli` is, by stated invariant in its own header, **dependency-free** (zero
...
Adopt **`react-docgen-typescript`** as a **dev dependency of `@elabs-ai/components-cli`**.
