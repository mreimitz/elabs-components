...
- **`@qlik-coe-emea/qlabs-components-flow`'s `CanvasShell`**
...
- **`@qlik-coe-emea/qlabs-components-ai`'s `Canvas`** (`packages/ai/src/canvas.tsx:1-24`) — a
  ...
  `@qlik-coe-emea/qlabs-components-ai`'s barrel header (`packages/ai/src/index.ts:1-7`) records
  that the whole package is vendored from **Vercel AI Elements**
  (`https://elements.ai-sdk.dev`), rewired onto `@qlik-coe-emea/qlabs-components-ui` primitives and
  `@qlik-coe-emea/qlabs-components-tokens` — the AI canvas set is part of that vendored surface, not a
...
`@qlik-coe-emea/qlabs-components-ai` canvas modules (`canvas.tsx`, `controls.tsx`, `edge.tsx`, `node.tsx`,
...
|                         | `@qlik-coe-emea/qlabs-components-flow` (`CanvasShell`)                                                  | `@qlik-coe-emea/qlabs-components-ai` (`Canvas`)                                    |
| ----------------------- | ------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
...
This keeps `@qlik-coe-emea/qlabs-components-ai` self-contained (per D5 /
`.claude/rules/scope-and-non-goals.md` — it is a vendored AI-Elements surface an app composes
without pulling in a second domain package) and keeps `@qlik-coe-emea/qlabs-components-flow` a
...
- **`@xyflow/react` appears in two packages BY DESIGN.** A future dependency-dedupe pass
  ...
  below.
- **The one-way dependency DAG is preserved.** `@qlik-coe-emea/qlabs-components-ai` and
  `@qlik-coe-emea/qlabs-components-flow` are both Layer-2 domain packages; `pnpm dep-direction:check`
...
- The AI canvas remains **unstoried** as of this ADR — filed separately (`area:ai`) as a
  ...
  (qlik-bright, qlik-dark, blueprint), respecting the eager-heavy-deps baseline.

## Alternatives considered and rejected

- **Consolidate onto one canvas implementation** (e.g. `@qlik-coe-emea/qlabs-components-ai`
  importing `CanvasShell` from `@qlik-coe-emea/qlabs-components-flow`). Rejected: it requires a
