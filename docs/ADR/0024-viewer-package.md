# ADR 0024 — `@qlik-coe-emea/qlabs-components-viewer` (adapter-registry leaf) + a shared file model in `@qlik-coe-emea/qlabs-components-ui`
...
### 1. Ship `@qlik-coe-emea/qlabs-components-viewer` as a Layer-2 leaf
...
**Why not inside `@qlik-coe-emea/qlabs-components-ai`.** Not for bundle weight — that
...
4. **Why not `@qlik-coe-emea/qlabs-components-data`.** That package is TanStack-table-shaped
...
### 4. The shared file model lives in `@qlik-coe-emea/qlabs-components-ui`
...
`@qlik-coe-emea/qlabs-components-ai` already barrel-exports `Toolbar`
