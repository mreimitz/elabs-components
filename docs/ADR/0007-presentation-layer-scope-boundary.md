...
As brand-ui added AI/agent surfaces (`@qlik-coe-emea/qlabs-components-ai`) and the generative-UI roadmap
...
swappable — never inside the `@qlik-coe-emea/qlabs-components-*` component packages.
...
- `@qlik-coe-emea/qlabs-components-ai` (and peers) stay presentational + runtime-agnostic; the app owns
