...
held 10 keys, and `@elabs-ai/components-ai` alone hardcoded ~100 user-visible English strings —
...
- **Package-specific microcopy is namespaced** `<pkg>.<area>.<key>` —
  ...
  flat bundle of ~100 `@elabs-ai/components-ai` keys would swamp the shared set and collide with
  future packages.

### 3. `DEFAULT_MESSAGES` stays in `@elabs-ai/components-ui`

Even though the keys belong to `@elabs-ai/components-ai`. This is correct under the one-way
...
- Remaining `@elabs-ai/components-ai` strings (JSX text nodes, default props) are **not yet**
...
- **A surface the original audit could not see:** `Streamdown` accepts a
  ...
  `externalLinkWarning`, …) that `@elabs-ai/components-ai` passes nothing for. Every
