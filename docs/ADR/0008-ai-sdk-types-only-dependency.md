...
`@qlik-coe-emea/qlabs-components-ai` renders the Vercel AI SDK data model — it `import type`s `UIMessage`,
...
51 `@qlik-coe-emea/qlabs-components-ai` source files** import from `ai`, **all `import type`, zero runtime
...
- `@qlik-coe-emea/qlabs-components-ai` may `import type` the message model from `ai`. It must **never** import
...
- Adding a runtime value import to `@qlik-coe-emea/qlabs-components-ai` now fails CI with a pointer to this
...
- Revisit trigger: a **vendor-neutral message standard**, or a **second message
  model** (A2UI/AG-UI) arriving in `@qlik-coe-emea/qlabs-components-ai`, would justify promoting the aliased
