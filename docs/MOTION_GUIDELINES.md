...
4. **JS / Motion** — call `useReducedMotion()` (from `@qlik-coe-emea/qlabs-components-tokens`) and stop
   loops / swap transforms for fades; or wrap the subtree in `<BrandMotionConfig>`
   (from `@qlik-coe-emea/qlabs-components-ai`). Motion's own `reducedMotion` only drops transform/layout,
   so looping properties (e.g. `Shimmer`'s `backgroundPosition`) must still branch.

## Built-in animated components (`@qlik-coe-emea/qlabs-components-ui`)
...
asserts the gate in a real browser (`pnpm --filter @qlik-coe-emea/qlabs-components-docs test-storybook
