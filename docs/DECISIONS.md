...
| You want…                                                                  | Use                                                             | Notes                                                   |
| -------------------------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------- |
| To build an app/screen _with_ components (you or the agent write the code) | **Build-with** — import `@elabs/components-*` / copy-own blocks | The default. ~99% of work. "Aware-of-library."          |
...
| ---------------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------ |
| A **conversation** (text, tool calls, reasoning, sources, files) | **AI SDK `UIMessage`** + `@elabs/components-ai` chat components | **Shipped.** The default.                              |
...
| **Ad-hoc agent JSX** (flexible, less safe)                       | **`JSXPreview`** (`@elabs/components-ai`)                       | **Shipped.** Escape hatch — prefer A2UI when it lands. |
...
- **`ai` (Vercel AI SDK): types-only, peer, never runtime.** `@elabs/components-ai` may `import type`
