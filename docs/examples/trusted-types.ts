 ...
 *   - `@elabs-ai/components-editor` — `mermaid-viewer`, `mermaid-diagram`, `markdown-academic/math`
 *   - `@elabs-ai/components-ai`'s `schema-display`
 *   - `new Worker("<url string>")` from Vite's `?worker` transform, which is how
 *     `@elabs-ai/components-editor/monaco-environment` starts Monaco's five language workers
 *
