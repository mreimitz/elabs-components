---
archetype: dashboard
intent: "KPI overview screen — metrics, charts and records laid out in one screen"
...
A metrics-first overview screen: a fixed, developer-laid-out grid of KPI tiles, charts and a
table. This playbook described a second archetype — an end-user-editable dashboard **sheet**,
a serializable `DashboardSpec` a person rearranges at runtime — until 2026-09-22, when that
surface was parked while its authoring experience is reworked (`parked/README.md`). Nothing
below depends on it.
...
`.claude/rules/charts.md` (chart choice, marks, honesty, responsive tiers) ·
`skills/brand-ui/reference/chart-selection.md` (which chart for which data shape) ·
`docs/REGISTRY_GUIDELINES.md` (copy-own blocks).
