...
| **`brand-ui-create-theme`** | maintainer + consumer | Research a brand from the links and files you give (plus official guidelines and the live product), capture its logo, and propose a sourced theme family; writes it only after approval. `/create-theme` in this repo.                                                                                                                                                             |
| **`brand-ui-update-theme`** | maintainer + consumer | Enhance an existing theme family from new material as a per-token diff with sources; edits only after approval. `/update-theme` in this repo.                                                                                                                                                                                                                                      |
...
```
npx skills add <path-to-this-repo>        # reads skills/ directly
```
