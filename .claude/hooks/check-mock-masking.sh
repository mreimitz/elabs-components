...
# the "mock-masking" smell: the file MOCKS a @qlik-coe-emea/qlabs-components-* component (vi.mock("@qlik-coe-emea/qlabs-components-..."))
...
# 1. mocks a @qlik-coe-emea/qlabs-components-* module (vi.mock / jest.mock / vi.doMock)
grep -Eq '(vi|jest)\.(do)?[mM]ock\([[:space:]]*["'"'"']@qlik-coe-emea/qlabs-components-' "$file_path" 2>/dev/null || exit 0
...
if grep -Eq "aria-|toHaveAttribute\([[:space:]]*['\"]aria|getByRole\([^)]*name" "$file_path" 2>/dev/null; then
  cat >&2 <<MSG
⚠ mock-masking check ($(basename "$file_path")): this file mocks a @qlik-coe-emea/qlabs-components-* component
