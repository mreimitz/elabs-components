...
# Warns when a @elabs-ai/components-ui component folder (packages/ui/src/components/<name>/) is
...
if ! grep -qF "./components/$name" "$barrel" 2>/dev/null; then
  cat >&2 <<MSG
⚠ component-registration: @elabs-ai/components-ui components/$name/ is not re-exported from src/index.ts.
