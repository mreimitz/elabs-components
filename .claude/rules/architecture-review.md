---
# Path-scoped so Claude Code does NOT auto-load this 14.7 KB rubric into EVERY session
# (the repo's intent: "NOT a CLAUDE.md always-import — it must not tax every session").
# The `repo-architect-*` agents + `/repo-architect-review` Read it EXPLICITLY ("Load
# .claude/rules/architecture-review.md") regardless of this `paths:` glob, so on-demand
# access is unaffected; the glob only stops the unintended always-on auto-discovery.
# Scoped to the arch-review machinery (not product code). See
# `.claude/rules/quality-gates.md` "Enforcement over reminders" and the `rules:scoping:check` gate.
paths:
  - ".claude/scripts/arch-evidence-pack.mjs"
  - "runs/**"
---
...
One-way deps (`tokens → ui/icons → data/ai/flow/maps/charts/marketing/editor/viewer/terminal → process`); no sideways/circular imports; packages match their charter. `process` is the single **layer-3** composite (ADR 0034) — it may depend on the layer-2 leaves `flow`/`charts`/`data` and nothing may depend on it; **primitives go down** into the base package that owns them (`flow` graph, `charts` marks/scales, `data` tables/filters, `ui` controls, `tokens` ramps) **and compositions go up**, so a leaf-to-leaf edge is still the violation it always was.
...
The built `dist/`: exports/types resolve in a consumer; tree-shaking; no dev leakage; both themes render right on a **real, unmodified** screen.
