...
- Media (ADR 0041): `AudioPlayer*` are presets over ui `MediaPlayer*` and emit the shared
  `media-player*` slots (no `audio-player*` selectors); `GeneratedImage` renders AI-SDK output, ui `Image` everything else —
