...
# detail is load-bearing when editing `@qlik-coe-emea/qlabs-components-ai`. See `.claude/rules/quality-gates.md`
...
| ------------------------------------------------------- | ---------------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------ |
| **AI SDK `UIMessage`**                                  | "what the agent **said**" — a conversation                 | the _content_ (your app owns the look)                      | **Shipped** — rendered by `@qlik-coe-emea/qlabs-components-ai` chat components |
...
| **`JSXPreview`** (`@qlik-coe-emea/qlabs-components-ai`) | ad-hoc agent **JSX markup strings**                        | flexible, _less safe_ ad-hoc rendering                      | **Shipped** — the escape hatch                                                 |
...
`@qlik-coe-emea/qlabs-components-ai` components are **presentational and runtime-agnostic**: they render the AI SDK data
...
- **Blocked (the failure the hook catches):** any _value_ import from `ai` or `@ai-sdk/*` —
  ...
  import; it must live in the app, never in `@qlik-coe-emea/qlabs-components-ai`.
