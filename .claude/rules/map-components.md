...
# Map components (@qlik-coe-emea/qlabs-components-maps)

`@qlik-coe-emea/qlabs-components-maps` wraps **MapLibre GL** (`maplibre-gl`, BSD-3-Clause) as brand-ui
components — the same wrap-an-engine pattern as `@qlik-coe-emea/qlabs-components-flow` (React Flow) and
`@qlik-coe-emea/qlabs-components-editor` (Monaco). Adapted from the MIT-licensed
...
- **Token paints (WebGL can't read CSS variables):** default layer paints
  resolve semantic tokens at runtime via `resolveTokenColor` (`@qlik-coe-emea/qlabs-components-tokens`)
...
- **Testing:** MapLibre can't render in jsdom — unit tests mock the engine
  (`src/test-utils/maplibre-mock.ts`, the `@qlik-coe-emea/qlabs-components-flow` pattern) and pure
