# Icons (Lucide is the default; `@qlik-coe-emea/qlabs-components-icons` is for brand vocabulary)
...
- **Default icon library: Lucide (`lucide-react`).** Generic UI glyphs — chevrons, `X`,
  ...
  **sanctioned** path, not a workaround (issue #300); the repo's own `MarkdownToolbar`
  does exactly this.
- **`@qlik-coe-emea/qlabs-components-icons` is for brand / product-vocabulary icons** — the product's own concept
  ...
  **placeholders** a product REPLACES with its own vocabulary — `@qlik-coe-emea/qlabs-components-icons` is
...
| A brand / product-concept icon, or the logo           | **`@qlik-coe-emea/qlabs-components-icons`** (`Icon`/`createIcon`, `BrandLogo`) |
...
- **One `lucide-react` version across the whole monorepo** (currently `^0.577.0`), declared as a
  normal `dependency` in each package/app whose source imports it (`@qlik-coe-emea/qlabs-components-ui`, `@qlik-coe-emea/qlabs-components-ai`,
  `@qlik-coe-emea/qlabs-components-editor`, the apps). `@qlik-coe-emea/qlabs-components-icons` does **not** depend on Lucide — it ships its own icons.

## Accessibility

- **Decorative icon → hidden from AT.** `@qlik-coe-emea/qlabs-components-icons`' `Icon` does this automatically when no
...
- **Other icon libraries are blocked by ESLint** (`no-restricted-imports` in
  `@qlik-coe-emea/qlabs-components-eslint-config`): importing heroicons / react-icons / tabler / phosphor / font-awesome /
  MUI-icons / ant-icons fails `pnpm lint`. Use `lucide-react` or `@qlik-coe-emea/qlabs-components-icons`.
