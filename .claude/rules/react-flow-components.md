...
# React Flow components (@qlik-coe-emea/qlabs-components-flow)
...
- **Not to be confused with `@qlik-coe-emea/qlabs-components-ai`'s `Canvas`.** That is a
  ...
  `@qlik-coe-emea/qlabs-components-ai`'s `Canvas` to render an agent's workflow graph live inside a
...
- **Attribution: hidden on both canvas surfaces — do NOT "restore" it.**
  `CanvasShell` (`@qlik-coe-emea/qlabs-components-flow`) and `Canvas`
  (`@qlik-coe-emea/qlabs-components-ai`) both pass
