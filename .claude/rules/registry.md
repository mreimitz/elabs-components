...
- `pnpm gen:registry` derives `files[]`, `files[].target`, `dependencies[]`,
...
- Never hand-write `files`/`dependencies`/`registryDependencies` — run `pnpm gen:registry`.
- After any change under `registry/`: `pnpm gen:registry && pnpm registry:validate &&
pnpm registry:resolve:check` (pre-commit enforces this).
