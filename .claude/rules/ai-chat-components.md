...
# AI / chat components (@qlik-coe-emea/qlabs-components-ai)

`@qlik-coe-emea/qlabs-components-ai` vendors **Vercel AI Elements** onto brand-ui — rewired to import
primitives from `@qlik-coe-emea/qlabs-components-ui` and tokens from `@qlik-coe-emea/qlabs-components-tokens`. Components are
...
- **Not to be confused with `@qlik-coe-emea/qlabs-components-flow`.** `Canvas`/`Node`/`Edge`/
  `Connection`/`Controls`/`Panel`/`Toolbar` are the **IN-CHAT agent workspace graph**;
  for an author-built diagram screen use `@qlik-coe-emea/qlabs-components-flow`'s `CanvasShell`
...
```tsx
const { t } = useLocale(); // from @qlik-coe-emea/qlabs-components-ui — provider-OPTIONAL
