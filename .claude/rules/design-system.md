...
- **Tokens are the source of truth.** Every visual decision is a semantic token
  in `@qlik-coe-emea/qlabs-components-tokens`. Components reference tokens, never raw values. Re-branding
...
- **Two consumption modes.** Stable primitives are _imported_ from `@qlik-coe-emea/qlabs-components-*`;
