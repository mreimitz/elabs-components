...
# Code editor components (@qlik-coe-emea/qlabs-components-editor)

`@qlik-coe-emea/qlabs-components-editor` wraps **Monaco** (the VS Code editor engine, `monaco-editor`) as
brand-ui components — the same wrap-an-engine pattern as `@qlik-coe-emea/qlabs-components-flow` (React
Flow) and `@qlik-coe-emea/qlabs-components-data` (TanStack). Monaco renders its own editing surface +
...
- **AI content-access API:** `EditorContentAccess` — engine-agnostic interface for AI-driven editing (get/replace selection, insert at cursor, subscribe to selection changes). `monacoContentAccess(editor)` adapter (`.` barrel + `./markdown`); `proseMirrorContentAccess(deps)` adapter (`./markdown` only — keeps `@milkdown` off the Monaco graph). `MarkdownEditorHandle` and `MarkdownWorkspaceHandle` extend `EditorContentAccess`; `CodeWorkspaceHandle` does too. Pass a raw `CodeEditor`/`DiffEditor` ref through `monacoContentAccess` for the same uniform API.
  ...
  `CodeBlock` from `@qlik-coe-emea/qlabs-components-ai` (Shiki) — `@qlik-coe-emea/qlabs-components-editor` is for **editing**.
- **Workers (required for IntelliSense):** import the worker setup ONCE at the app
  entry — `import "@qlik-coe-emea/qlabs-components-editor/monaco-environment";` (Vite). Without it the
...
- **Theming:** never pass Monaco a hardcoded theme. The bridge
  ...
  Tokens are oklch → resolved to hex via the shared `oklchToHex` from `@qlik-coe-emea/qlabs-components-tokens`
...
- **Story coverage & verification:** cover the key states (languages, read-only,
  ...
  `pnpm --filter @qlik-coe-emea/qlabs-components-docs test-storybook`. Monaco can't render in jsdom — unit
...
`@qlik-coe-emea/qlabs-components-editor` ships a **markdown authoring + preview suite** under the
`@qlik-coe-emea/qlabs-components-editor/markdown` subpath — kept off the main barrel so Monaco-only
...
`import { MarkdownWorkspace, MarkdownPreview } from "@qlik-coe-emea/qlabs-components-editor/markdown";`.

- **Surfaces (exported from `@qlik-coe-emea/qlabs-components-editor/markdown`):**
...
privileged built-in). Inline directives render via a separate`brand-directive-inline` tag so they stay in the text flow.
  ...
  - **Frontmatter utils** live one level deeper on `@qlik-coe-emea/qlabs-components-editor/markdown/frontmatter`
  ...
  - **Markdown parser** lives on `@qlik-coe-emea/qlabs-components-editor/markdown/parse` (`parseMarkdown(md): Root`) —
  ...
  - **Academic layer (opt-in props on `MarkdownPreview`)** — footnotes, citations +
    ...
    - `@qlik-coe-emea/qlabs-components-editor` cannot import `@qlik-coe-emea/qlabs-components-ai` (sibling in the one-way dep graph), so
      the inline cite is composed from `@qlik-coe-emea/qlabs-components-ui` + a native `title` hover — NOT
      `@qlik-coe-emea/qlabs-components-ai`'s `InlineCitation`.
  - **Iteration (opt-in props on `MarkdownPreview`)** — `:::iterate` / `:::pivot`
    ...
    - The directive BODY is the template, captured RAW via the seam's new
      ...
      via the shared `@qlik-coe-emea/qlabs-components-ui` `Table` — `@qlik-coe-emea/qlabs-components-editor` can't import the sibling
      `@qlik-coe-emea/qlabs-components-data`'s DataTable, so it composes the shared primitive (same base), not a
...
- **When to use which:**
  ...
  - **Read-only display of a CODE block** (syntax highlight, not prose) →
    `CodeBlock` from `@qlik-coe-emea/qlabs-components-ai` (Shiki). Don't reach for an external markdown lib —
...
unit-tested; the live decoration passes come from `pnpm --filter @qlik-coe-emea/qlabs-components-docs
