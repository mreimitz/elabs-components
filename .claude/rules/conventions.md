...
- Set type with a role (`text-display-lg|display|title|subtitle|body|caption|meta|eyebrow|kpi|kpi-sm|code` or `<Heading>`/`<Text>`), never a raw size utility (`text-sm`, `text-xl`, `text-[17px]`) in package source or stories. (`text-scale`)
...
- Type is a role: use `text-display-lg|display|title|subtitle|body|caption|meta|eyebrow|kpi|kpi-sm|code`, never raw `text-sm`/`text-[17px]` in `packages/*/src/**/*.tsx` (stories are covered by the `text-scale` rule). (`type-roles`)
...
- Build the website (`apps/home`, ADR 0038) only from the library: import react, react-dom, next, motion, @vercel/analytics, `@elabs-ai/*` or a relative path; a registry block copied under `apps/home/components/blocks/` keeps its `// registry: <name> — copied <YYYY-MM-DD>` header. (`home-imports`)
