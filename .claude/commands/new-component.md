...
1. **Search for the requested capability** — by name AND by concept, not just an
   ...
   - **If the Storybook dev server is running**, call
     ...
     every documented component across all `@elabs-ai/components-*` packages, then
...
5. **Don't duplicate non-component primitives either:** a `cn` helper already
   exists (`@elabs-ai/components-ui/lib/cn`), icons live in `@elabs-ai/components-icons`, tokens in
   `@elabs-ai/components-tokens`. Reuse them; never re-add a local copy.
...
1. Create `packages/<pkg>/src/components/<kebab-name>/` (for `@elabs-ai/components-ui`) or
