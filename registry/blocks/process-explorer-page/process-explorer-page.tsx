/**
 * Process explorer page (copy-owned block, RM-057). The `process-explorer` archetype:
 * a KPI strip on top, a filter bar above the map, the map centred with its accessible
 * table twin, a variant rail on the right, and a case table that opens a single case's
 * timeline in a bottom sheet. One `useProcessExplorer` instance drives every panel, so
 * the map, the filter chips, the KPI numbers and the variant rail can never disagree
 * about what is currently in scope.
 * Remember to `import "@xyflow/react/dist/style.css"` once at the app root.
 * Depends on installed @elabs-ai/components-process + @elabs-ai/components-flow +
...
export function ProcessExplorerPage() {
  ...
  // (the map and the rail write to the same `useProcessExplorer` instance, so there is
  // only one intent list to read).
  ...
  return (
    ...
    <div className="flex h-full min-h-0 flex-col gap-3 overflow-hidden bg-background p-6">
      ...
      {/* Case drill-down: the table opens in a bottom sheet; opening a row swaps the
          sheet's own body to that case's timeline (drill path is always variant → case
          table → single case). */}
      <Sheet
        open={casesOpen}
        onOpenChange={(open) => {
