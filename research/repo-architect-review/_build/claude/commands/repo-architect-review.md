...
- `/file-issue` — hand `runs/<date>/findings.md` (or the single record) to **`/file-issue`**
  (→ `root-cause-analyst` deep RCA → dedupe → GitHub issue). Architecture-scale findings
...
- component/a11y tier → recommend `/review-component` / `accessibility-reviewer` instead of
