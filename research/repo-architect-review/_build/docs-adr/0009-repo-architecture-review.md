...
rendered surface (`visual-ux-reviewer`, `accessibility-reviewer`, `/qa-flows`), and agent
process (`/session-retro`) — plus a point-in-time advisory architect
(`design-system-architect`). What it lacked was a **recurring, repo/system-tier** review:
...
4. **gates** (manager approves) — then emits the two surfaces and routes approved findings
   to the existing `/file-issue` → `root-cause-analyst` spine.
...
- **Dedicated synthesizer, not `design-system-architect`.** Keeps the audit self-contained
  and portable (the plugin goal) and read-only (cleaner finder posture). `design-system-architect`
...
- **Plugin-scoped names** (`repo-architect-*`, `/repo-architect-review`). Avoids the
  ...
  `design-system-architect` collide across installed marketplaces).
...
- **Reuse `design-system-architect` as synthesizer** — rejected: couples the audit to
